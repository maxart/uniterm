...
pub fn run() -> i32 {
    ...
    let rc = match args.first().map(String::as_str) {
        ...
        Some("attach" | "att" | "a") => cmd_attach(&args[1..]),
        ...
        Some("today" | "timeline") => cmd_today(&args[1..]),
        Some("privacy") => cmd_privacy(&args[1..]),
...
fn cmd_serve(args: &[String]) -> i32 {
    ...
    let notify = args.get(1).is_some_and(|arg| arg == "--startup-notify");
    if !notify {
        println!("uniterm: serving '{name}' at {}", sock.display());
        println!("uniterm: attach from another terminal with:  uniterm attach {name}");
    }
    match uniterm_server::server::run_server_with_ready(&sock, &shell(), &[], || {
        if notify {
...
fn ensure_workspace_available(name: &str) -> Result<PathBuf, String> {
    ...
    startup::wait_for_server(&mut child, name).map_err(|error| error.to_string())?;
    Ok(sock)
}
...
fn cmd_new_session(args: &[String]) -> i32 {
    ...
    if let Err(error) = startup::wait_for_server(&mut child, name) {
        eprintln!("uniterm workspace new: {error}");
        return 1;
    }
    if detached {
...
fn cmd_project(args: &[String]) -> i32 {
    ...
    let request = match args.first().map(String::as_str) {
        ...
        Some("add" | "new") => {
            if args.len() > 4 || args.get(3).is_some_and(|flag| flag != "--background") {
                eprintln!("usage: ut project add NAME ROOT [--background] [-w Workspace]");
                return 2;
            }
            ...
            if args.iter().any(|value| value == "--background") {
                return control_mutation(
                    &sock,
                    uniterm_proto::ControlCommand::ProjectCreate {
                        name: name.clone(),
...
fn cmd_pane(args: &[String]) -> i32 {
    ...
    match args.first().map(String::as_str) {
        ...
        Some(action @ ("split" | "close")) => {
            let pane = match resolve_pane_target(args.get(1).map(String::as_str)) {
                Ok(pane) => pane,
                Err(error) => {
                    eprintln!("uniterm pane {action}: {error}");
                    return 2;
                }
            };
            if args.iter().skip(2).any(|value| {
                action == "close" || !matches!(value.as_str(), "--vertical" | "--background")
            }) {
                eprintln!(
                    "usage: ut pane split PANE [--vertical] [--background] | ut pane close PANE"
                );
                return 2;
            }
            let command = if action == "close" {
                uniterm_proto::ControlCommand::PaneClose { pane }
            } else {
                uniterm_proto::ControlCommand::PaneSplit {
                    pane,
                    axis: if args.iter().any(|value| value == "--vertical") {
                        uniterm_proto::SplitAxis::TopBottom
                    } else {
                        uniterm_proto::SplitAxis::LeftRight
                    },
                    background: args.iter().any(|value| value == "--background"),
                }
            };
            control_mutation(&socket, command, action == "split")
        }
        ...
        Some("submit") => {
            let pane = match resolve_pane_target(args.get(1).map(String::as_str)) {
                Ok(pane) => pane,
                Err(error) => {
                    eprintln!("uniterm pane submit: {error}");
                    return 2;
                }
            };
            paste_to_pane(&socket, pane, "", true)
        }
        Some("send-keys" | "send" | "paste" | "prompt") => {
            let Some(value) = args.get(1) else {
            ...
            let submit = args[0] == "prompt" || args.iter().skip(3).any(|value| value == "--enter");
            if submit || args[0] == "paste" {
                return paste_to_pane(&socket, pane, text, submit);
            }
        ...
        _ => {
            eprintln!("uniterm pane: usage: ut pane <list|focus|read|send-keys|paste|prompt|submit|split|close|wait-output|metadata> ... [-w Workspace]");
            2
        }
    }
}
...
/// Stdin avoids shell interpolation and preserves multiline prompts verbatim.
fn prompt_text(value: &str) -> Result<String, std::io::Error> {
    if value == "--stdin" {
        ...
        if text.len() > 256 * 1024 {
            return Err(std::io::Error::other("prompt exceeds 256 KiB"));
        }
        Ok(text)
    } else {
        Ok(value.to_string())
...
/// The server knows the destination's paste mode; callers never guess keys.
fn paste_to_pane(
    ...
    let text = match prompt_text(text) {
        Ok(text) => text,
        Err(error) => {
            eprintln!("uniterm: {error}");
            return 1;
    ...
    match uniterm_client::control_request(
        socket,
        uniterm_proto::ControlCommand::PanePaste { pane, text, submit },
    ) {
        Ok(response) => match response.result {
            ...
            _ => {
                eprintln!("uniterm: Pane missing or input refused; prompt was not queued");
                1
            }
        },
        Err(error) => {
...
/// Automation mutations report acceptance and stable identities without attaching.
fn control_mutation(
    socket: &std::path::Path,
    command: uniterm_proto::ControlCommand,
    print_id: bool,
) -> i32 {
    match uniterm_client::control_request(socket, command) {
        Ok(response) => match response.result {
            Some(uniterm_proto::ControlResult::Mutation {
                accepted: true, id, ..
            }) => {
                if let Some(id) = id.filter(|_| print_id) {
                    println!("{id}");
                }
                0
            }
            _ => {
                eprintln!(
                    ...
                    response.error.map_or_else(
                        || "target missing or operation refused".into(),
                        |error| error.message
                    )
                );
                1
...
fn cmd_privacy(args: &[String]) -> i32 {
    let result = match args {
        [verb, path] if verb == "keygen" => {
            uniterm_server::privacy::generate_key(PathBuf::from(path).as_path())
                .map_err(|e| e.to_string())
        }
        [verb, name] if verb == "status" => {
            if let Err(e) = uniterm_proto::validate_workspace_name(name) {
                eprintln!("{e}");
                return 2;
            }
            println!(
                "{}",
                if uniterm_server::privacy::is_protected(name) {
                    "encrypted; external key required"
                } else {
                    "local owner-only files; not encrypted"
                }
            );
            return 0;
        }
        [verb, name, key_file] if verb == "protect" => {
            let socket = match checked_socket_path(name) {
                Ok(socket) => socket,
                Err(e) => {
                    eprintln!("{e}");
                    return 2;
            ...
            let _claim = match lock_stopped_workspace(name, &socket) {
                Ok(claim) => claim,
                Err(e) => {
                    eprintln!("{e}");
                    return 1;
            ...
            uniterm_server::privacy::protect(name, PathBuf::from(key_file).as_path())
                .map_err(|e| e.to_string())
        }
        _ => {
            eprintln!(
                "usage: ut privacy keygen KEY_FILE | status WORKSPACE | protect WORKSPACE KEY_FILE"
            );
            return 2;
        }
    };
    match result {
        Ok(()) => {
            println!("Done. Keep the external key backed up; set UNITERM_STATE_KEY_FILE to its path when opening protected Workspaces.");
            0
        }
        Err(error) => {
            eprintln!("uniterm privacy: {error}");
            1
        }
    }
}

fn cmd_today(args: &[String]) -> i32 {
    use uniterm_proto::{ControlCommand, ControlResult};
    let (socket, args) = match project_socket(args) {
        Ok(parsed) => parsed,
        Err(error) => {
            eprintln!("uniterm today: {error}");
            return 2;
        }
    };
    if args.first().is_some_and(|a| a == "watch") {
        let after = if args.len() == 1 {
            0
        } else if args.len() == 3 && args[1] == "--after" {
            match args[2].parse() {
                Ok(n) => n,
                Err(_) => return 2,
            }
        } else {
            eprintln!("usage: ut today watch [--after SEQUENCE]");
            return 2;
        };
        return match watch_today(&socket, after) {
            Ok(()) => 0,
            Err(e) => {
                eprintln!("uniterm today watch: {e}");
                1
            }
        };
    }
    if args.first().is_some_and(|a| a == "manager") {
        if args.len() < 3
            || args
                .iter()
                .skip(3)
                .any(|a| a != "--manage" && a != "--background")
        {
            eprintln!("usage: ut today manager PROVIDER PROJECT_ID [--manage] [--background]\nShares Project paths, session IDs and activity metadata with the chosen provider. --manage also grants normal Workspace control and private content access.");
            return 2;
        }
        let Ok(project) = args[2].parse::<u64>() else {
            return 2;
        };
        return control_mutation(
            &socket,
            ControlCommand::TimelineManager {
                agent: args[1].clone(),
                project: uniterm_core::ProjectId(project),
                manage: args.iter().any(|a| a == "--manage"),
                background: args.iter().any(|a| a == "--background"),
            },
            true,
        );
    }
    if args.first().is_some_and(|a| a == "prompt") && args.len() == 4 && args[3] == "--retain" {
        let Ok(pane) = args[1].parse::<u64>() else {
            return 2;
        };
        return control_mutation(
            &socket,
            ControlCommand::TimelinePrompt {
                pane: uniterm_core::PaneId(pane),
                text: match prompt_text(&args[2]) {
                    Ok(text) => text,
                    Err(e) => {
                        eprintln!("uniterm today prompt: {e}");
                        return 1;
                    }
                },
            },
    ...
    if args.first().is_some_and(|a| a == "resume") && args.len() == 2 {
        let Ok(sequence) = args[1].parse::<u64>() else {
            return 2;
        };
        return control_mutation(&socket, ControlCommand::TimelineResume { sequence }, true);
    }
    if args.is_empty() || args.as_slice() == ["open"] {
        return control_mutation(&socket, ControlCommand::TimelineOpen, false);
    }
    if args.first().is_some_and(|a| a == "note") && args.len() >= 3 {
        let Ok(project) = args[1].parse::<u64>() else {
            eprintln!("uniterm today note: PROJECT must be an id from ut project list");
            return 2;
        };
        return control_mutation(
            &socket,
    ...
    if args.first().is_some_and(|a| a == "focus") && args.len() == 3 {
        let (Ok(pane), Ok(invocation)) = (args[1].parse::<u64>(), args[2].parse::<u64>()) else {
            eprintln!("uniterm today focus: expected PANE INVOCATION");
            return 2;
        };
        return control_mutation(
            &socket,
    ...
    if args.first().is_some_and(|a| a == "list") {
        ...
        while let Some(arg) = arguments.next() {
            match arg.as_str() {
                "--json" => json = true,
                "--filter" => {
                    filter = arguments.next().cloned().unwrap_or_default();
                }
                "--before" => {
                    before = arguments.next().and_then(|v| v.parse().ok());
                    if before.is_none() {
                        eprintln!("--before requires a sequence");
                        return 2;
                    }
                }
                value if !value.starts_with('-') && date == "today" => date = value.into(),
                _ => {
                    eprintln!("usage: ut today list [YYYY-MM-DD] [--json] [--before SEQUENCE]");
                    return 2;
                }
            }
        }
        match uniterm_client::control_request(
            ...
            Ok(response) => {
                if let Some(ControlResult::Timeline { day }) = response.result {
                    if json {
                        println!("{}", serde_json::to_string(&day).unwrap());
                    } else {
                        println!(
                            "{} {} | local metadata | through event {}",
                            day.date, day.timezone, day.current_sequence
                        );
                        ...
                        if let Some(before) = day.next_before {
                            println!("More history: --before {before}");
                        }
                    }
                    return 0;
                }
                eprintln!(
                    ...
                    response
                        .error
                        .map_or_else(|| "unexpected response".into(), |e| e.message)
                );
            }
            Err(error) => eprintln!("uniterm today: {error}"),
        }
        return 1;
    }
    eprintln!("usage: ut today [open|list [DATE] [--filter TEXT] [--json] [--before SEQ]|note PROJECT TEXT|focus PANE INVOCATION|resume SEQ|watch [--after SEQ]|manager PROVIDER PROJECT [--manage] [--background]|prompt PANE TEXT --retain] [-w Workspace]");
    2
}

fn watch_today(socket: &std::path::Path, after_sequence: u64) -> std::io::Result<()> {
    use std::io::{BufRead, Read, Write};
    let workspace = socket
        .file_stem()
        .and_then(|s| s.to_str())
        .ok_or_else(|| std::io::Error::other("invalid Workspace socket"))?;
    let mut stream =
        std::os::unix::net::UnixStream::connect(socket.with_extension("control.sock"))?;
    let request = uniterm_proto::ControlRequest {
        version: uniterm_proto::CONTROL_API_VERSION,
        id: 1,
        workspace: workspace.into(),
        command: uniterm_proto::ControlCommand::Subscribe { after_sequence },
    };
    writeln!(stream, "{}", serde_json::to_string(&request)?)?;
    let mut reader = std::io::BufReader::new(stream);
    loop {
        let mut line = Vec::new();
        // A corrupt peer must not grow the monitor without a bound.
        reader
            .by_ref()
        ...
        if line.is_empty() {
            return Err(std::io::Error::new(
                ...
                "stream closed; reconnect after the last event sequence",
            ));
        }
        if line.len() > uniterm_proto::CONTROL_MAX_FRAME_BYTES as usize || !line.ends_with(b"\n") {
            return Err(std::io::Error::other("invalid control frame"));
        }
        let frame: uniterm_proto::ControlFrame = serde_json::from_slice(&line)?;
        if let uniterm_proto::ControlFrame::Response(response) = &frame {
        ...
        stdout.flush()?;
        if matches!(frame, uniterm_proto::ControlFrame::StreamError(_)) {
            return Err(std::io::Error::other(
                "subscription failed; inspect the stream error",
            ));
        }
    }
}
...
fn cmd_tab(args: &[String]) -> i32 {
    ...
    let background_move = args.first().map(String::as_str) == Some("move")
        && args.iter().any(|arg| arg == "--background");
    let args: Vec<String> = args
        .into_iter()
        .filter(|arg| !background_move || arg != "--background")
        .collect();
    if background_move && args.len() != 4 {
        eprintln!("uniterm tab move: --background requires PROJECT TAB left|right");
        return 2;
    }
    if args.first().map(String::as_str) == Some("move") && args.len() == 4 {
        let (project, _, tab) = match resolve_hierarchy_location(&socket, &args[1], &args[2]) {
            Ok(target) => target,
            Err(error) => {
        ...
        let direction = match args[3].as_str() {
            "left" | "previous" | "prev" => uniterm_proto::TabMoveDirection::Previous,
            "right" | "next" => uniterm_proto::TabMoveDirection::Next,
            _ => {
                eprintln!("usage: ut tab move PROJECT TAB left|right");
                return 2;
            }
        };
        return control_mutation(
            &socket,
            uniterm_proto::ControlCommand::TabMove {
                project,
                tab,
                direction,
            },
    ...
    if args.first().map(String::as_str) == Some("new") {
        let background = args.iter().any(|value| value == "--background");
        let args: Vec<_> = args
            .iter()
            .filter(|value| value.as_str() != "--background")
            .collect();
        if args.len() > 2 {
            eprintln!("uniterm tab new: usage: ut tab new [project] [--background] [-w Workspace]");
            return 2;
        }
        ...
        return match uniterm_client::control_request(
            &socket,
            uniterm_proto::ControlCommand::TabCreate {
                project,
                background,
            },
        ) {
    ...
    if matches!(
        ...
        Some("paste" | "prompt" | "submit")
    ) {
        let submit_only = args[0] == "submit";
        if args.len() != if submit_only { 3 } else { 4 } {
            eprintln!(
                "usage: ut tab paste|prompt PROJECT TAB TEXT|--stdin; ut tab submit PROJECT TAB"
            );
            return 2;
        }
        let (project, _, tab) = match resolve_hierarchy_location(&socket, &args[1], &args[2]) {
        ...
        let text = if submit_only {
            String::new()
        } else {
            match prompt_text(&args[3]) {
                Ok(text) => text,
                Err(error) => {
                    eprintln!("uniterm tab: {error}");
                    return 1;
        ...
        return control_mutation(
            &socket,
            uniterm_proto::ControlCommand::TabPaste {
                project,
                tab,
                text,
                submit: args[0] != "paste",
    ...
    if args.first().map(String::as_str) == Some("close") {
        if args.len() != 3 {
            eprintln!("usage: ut tab close PROJECT TAB [-w Workspace]");
            return 2;
        }
        let (project, _, tab) = match resolve_hierarchy_location(&socket, &args[1], &args[2]) {
            Ok(target) => target,
            Err(error) => {
                eprintln!("uniterm tab close: {error}");
                return 1;
            }
        };
        return control_mutation(
            &socket,
            uniterm_proto::ControlCommand::TabClose { project, tab },
            false,
        );
    }
    ...
    match args.first().map(String::as_str) {
        ...
        Some("start") => {
            let Some(agent) = args.get(1) else {
                eprintln!("uniterm agent start: usage: ut agent start NAME [--current|--tab] [--pane PANE] [--background]");
                return 2;
            };
            if args.iter().skip(2).any(|value| value == "--current")
                && args.iter().skip(2).any(|value| value == "--tab")
            {
                eprintln!("uniterm agent start: choose either --current or --tab");
                return 2;
            }
            let target = if args.iter().skip(2).any(|value| value == "--current") {
            ...
            while let Some(option) = options.next() {
                match option.as_str() {
                    ...
                    "--current" | "--tab" => {}
                    "--pane" => {
                        pane = match options.next().and_then(|value| value.parse::<u64>().ok()) {
                            Some(id) => Some(uniterm_core::PaneId(id)),
                            None => {
                                eprintln!("uniterm agent start: --pane requires a Pane id");
                                return 2;
                            }
                        };
                    }
                    _ => {
                        eprintln!("uniterm agent start: unknown option '{option}'");
                        return 2;
                    }
                }
            }
            control_mutation(
                &socket,
                uniterm_proto::ControlCommand::AgentLaunch {
                    agent: agent.clone(),
                    target,
                    pane,
                    background,
        ...
        Some("prompt" | "send-keys") => {
            ...
            if args[0] == "prompt" || args.iter().skip(3).any(|value| value == "--enter") {
                return paste_to_pane(&socket, pane, text, true);
            }
            let bytes = text.as_bytes().to_vec();
            match uniterm_client::pane_send(&socket, pane, bytes) {
                Ok(Some(true)) => 0,
...
fn forget_stopped_workspace(name: &str) -> Result<bool, String> {
    ...
    uniterm_server::privacy::forget(name).map_err(|error| error.to_string())?;
    Ok(true)
}
...
fn print_help() {
    println!(
        "uniterm {} - a terminal multiplexer built for agentic engineering\n\
         ...
         \x20 uniterm attach [name]             attach to an existing Workspace (aliases: att, a)\n\
         ...
         \x20 uniterm today [list|note|watch|manager|resume|prompt] local day history\n\
         \x20 uniterm privacy keygen|protect|status ... optional state protection\n\
         ...
         \x20 uniterm pane paste|prompt|submit ... paste text or press Enter\n\
         \x20 uniterm tab focus PROJECT TAB     focus a Tab by name or ordinal\n\
         \x20 uniterm tab new [PROJECT] [--background] create a Tab\n\
         \x20 uniterm tab rename PROJECT TAB NAME name a Tab\n\
         \x20 uniterm tab move [PROJECT TAB] left|right reorder Tabs\n\
...
fn print_help_search(query: &str) -> i32 {
    ...
    let topics = [
        (
            "workspace attach att a",
            "ut attach [NAME] (aliases: att, a); attach to an existing Workspace",
        ),
        (
            "pane automation read output send keys wait split close",
            "ut pane list|read|send-keys|paste|prompt|submit|wait-output ...; ut pane split PANE [--vertical] [--background]; ut pane close PANE",
        ),
        (
        ...
        (
            "tab focus reorder move create close",
            "ut tab focus PROJECT TAB | ut tab new [PROJECT] [--background] | ut tab rename PROJECT TAB NAME | ut tab move [PROJECT TAB] left|right | ut tab close PROJECT TAB",
        ),
        (
        ...
        (
            "background automation create project tab pane agent start",
            "Use --background with: ut project add NAME ROOT; ut tab new [PROJECT]; ut pane split PANE; ut agent start NAME --pane PANE [--tab]. Explicit targets preserve the human's selection. Read and send never need focus. See ut --skill.",
        ),
        (
            "prompt paste submit enter return send text pane tab agent stdin multiline",
            "ut agent prompt PANE TEXT|--stdin; ut pane prompt PANE TEXT|--stdin; ut tab prompt PROJECT TAB TEXT|--stdin. These paste then press Enter (CR) outside bracketed paste. Use pane/tab paste for text only, pane submit PANE or tab submit PROJECT TAB for Enter only. Never type the word Enter or append a newline to submit. Wait for readiness, then verify output; acceptance means queued, not completed. See ut --skill.",
        ),
