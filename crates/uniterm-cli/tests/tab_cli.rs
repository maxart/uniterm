...
fn tabs_can_be_created_and_named_by_hierarchy_position() {
    ...
    human
        ...
        .unwrap();
    assert!(
        !drain_render_frames(&mut human, &AtomicBool::new(true)).is_empty(),
        "first attach must paint"
    );
    // A real terminal consumes frames throughout the commands. Leaving them
    // unread fills macOS's smaller socket buffer and deliberately invokes the
    // slow-client recovery repaint, unrelated to background focus changes.
    let stop_reading = Arc::new(AtomicBool::new(false));
    let reader_stop = Arc::clone(&stop_reading);
    let mut reader_stream = human.try_clone().unwrap();
    let human_reader = thread::spawn(move || drain_render_frames(&mut reader_stream, &reader_stop));
    ...
    let anchor = snapshot()
        ...
        .0
        .to_string();
    run(&["tab", "move", &project, "Work", "left", "--background"]);
    assert_eq!(
        snapshot()
            .into_iter()
            .find(|pane| pane.id.0.to_string() == anchor)
            .unwrap()
            .tab,
        1
    );
    ...
    let frames = human_reader.join().unwrap();
    assert!(
        frames
            .iter()
