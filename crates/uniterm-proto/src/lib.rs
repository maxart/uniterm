...
pub enum ControlCommand {
    /// Read a bounded page of local-day evidence without exposing raw prompts.
    TimelineQuery {
        date: String,
        #[serde(default)]
        filter: String,
    ...
    /// Save a deliberate breadcrumb in the owning Workspace event stream.
    TimelineNote {
        project: ProjectId,
        #[serde(default)]
        pane: Option<PaneId>,
    ...
    /// Start a provider in a new Tab with an explicit Workspace capability scope.
    TimelineManager {
        agent: String,
        project: ProjectId,
        manage: bool,
    ...
    /// Explicit semantic prompt retention requires protected storage.
    TimelinePrompt {
        pane: PaneId,
        text: String,
    },
    /// Explicitly resume a recorded provider session into a new Tab.
    TimelineResume {
        sequence: u64,
    },
    /// Reveal Today without changing the remembered Project or Pane.
    TimelineOpen,
    /// A historical jump must still identify the same live invocation.
    TimelineFocus {
        pane: PaneId,
        invocation: u64,
    },
    ...
    /// Paste literal text using the target terminal's negotiated paste mode.
    /// Submit presses Enter (CR) outside the paste; an empty text submits an
    /// already typed prompt without inserting another character.
    PanePaste {
        pane: PaneId,
        text: String,
        #[serde(default)]
    ...
    ProjectCreate {
        name: String,
        root: String,
        /// Preserve the attached human's selection when creating the Project.
        #[serde(default)]
        background: bool,
    },
    ProjectRename {
    ...
    /// Paste into the Tab's remembered active Pane without selecting the Tab.
    TabPaste {
        project: ProjectId,
        tab: u32,
        text: String,
    ...
    /// Close a targeted Tab without selecting it first.
    TabClose {
        project: ProjectId,
        tab: u32,
    },
    /// Split an explicit Pane without needing to select its Project or Tab.
    PaneSplit {
        pane: PaneId,
        axis: SplitAxis,
        #[serde(default)]
    ...
    /// Close a targeted Pane without selecting it first.
    PaneClose {
        pane: PaneId,
    },
    TabRename {
    ...
    AgentLaunch {
        agent: String,
        target: LaunchTarget,
        /// Anchor the launch to this Pane instead of the human's selection.
        #[serde(default)]
        pane: Option<PaneId>,
        #[serde(default)]
        background: bool,
...
pub enum ControlResult {
    Timeline {
        day: Box<uniterm_core::timeline::TimelineDay>,
    },
    Capabilities {
...
/// Historical resume evidence crosses the runtime seam only after an explicit request.
#[derive(Clone, Debug)]
pub struct TimelineResumeData {
    /// Capability scope follows the recorded invocation across restart/resume.
    pub manager_read_only: bool,
    pub context: uniterm_core::timeline::TimelineContext,
    pub predecessor: u64,
    pub provider: String,
...
pub enum CoreToAgent {
    /// Build/query the history index on a blocking worker, never the core loop.
    TimelineQuery {
        name: String,
        date: String,
        filter: String,
    ...
    /// Resolve historical resume evidence and check its directory on the runtime.
    TimelineResume {
        name: String,
        sequence: u64,
        requester: TimelineRequester,
    },
    /// Event-driven refresh exists only while the Today surface is visible.
    TimelineWatch { enabled: bool },
    ...
    /// Start an external editor after the core confirms the selected Project.
    /// Its process lifetime belongs to the runtime, without a terminal Tab.
    EditorLaunchExternal {
        project: ProjectId,
        path: String,
        command: String,
        cwd: String,
    },
    /// Persist an owned compact capture. Grapheme expansion, serialization,
    /// and filesystem I/O all stay on the runtime side of the seam.
    SnapshotSave {
        name: String,
        snapshot: Box<checkpoint::Snapshot<uniterm_core::GridCapture>>,
    },
    ...
    /// Stop only listeners owned by this Pane's PTY session, off the core loop.
    DevServerStop {
        pane: PaneId,
        port: u16,
        detected: u64,
...
pub enum AgentToCore {
    /// A bounded immutable result for the UI or one control request.
    TimelineLoaded {
        requester: TimelineRequester,
        result: Result<uniterm_core::timeline::TimelineDay, String>,
    },
    /// Provider-owned resume facts resolved from the authoritative history.
    TimelineResumeReady {
        requester: TimelineRequester,
        result: Result<TimelineResumeData, String>,
    },
    /// A committed meaningful observation invalidated the visible day.
    TimelineChanged {
        sequence: u64,
    },
    /// A catalog write completed. Only successful acknowledgements may enter
    /// the core's deduplication cache; failures remain eligible for retry.
    WorkspaceCatalogSaved {
        name: String,
        line: String,
        error: Option<String>,
    },
    ...
    EditorResolved {
        project: ProjectId,
        path: String,
        /// Canonical identity for reuse, preserving the requested launch path.
        canonical_path: String,
        command: String,
        /// External GUI launchers retain their own file/window policy.
        reuse_tab: bool,
        error: Option<String>,
    },
    /// Report external editor failures in Files without creating a terminal.
    EditorExternalFinished {
        project: ProjectId,
        error: Option<String>,
    },
    ...
    /// Completion of a user-requested stop; stale detection generations are ignored.
    DevServerStopFinished {
        pane: PaneId,
        port: u16,
        detected: u64,
        error: Option<String>,
...
pub enum Command {
    ...
    LastPane,
    /// Open the workspace-level day timeline (docs/27).
    Today,
}

/// Query generations keep slow history reads from replacing a newer selected day.
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq)]
pub enum TimelineRequester {
    View { generation: u64 },
    Control { connection: u64, id: u64 },
}
...
pub struct WorkspaceDefinition {
    ...
    #[serde(default = "WorkspaceDefinition::default_scope_workspace")]
...
impl WorkspaceDefinition {
    // Old catalogs without a saved choice use the same scope as new Workspaces.
    // Explicit saved false values still restore the user's Project selection.
    const fn default_scope_workspace() -> bool {
        true
    }
...
mod tests {
    ...
    #[test]
    fn workspace_scope_defaults_only_apply_when_no_choice_was_saved() {
        let mut catalog = serde_json::json!({
            "version": 2, "active_project": 1, "projects": []
        });
        let defaults: WorkspaceDefinition = serde_json::from_value(catalog.clone()).unwrap();
        assert!(defaults.agent_scope_workspace);
        assert!(defaults.server_scope_workspace);
        for (agents, servers) in [(false, true), (true, false), (false, false)] {
            catalog["agent_scope_workspace"] = agents.into();
            catalog["server_scope_workspace"] = servers.into();
            let saved: WorkspaceDefinition = serde_json::from_value(catalog.clone()).unwrap();
            assert_eq!(saved.agent_scope_workspace, agents);
            assert_eq!(saved.server_scope_workspace, servers);
        }
    }
    ...
    fn guardrail_settings_round_trip_on_current_wire_protocol() {
        assert_eq!(WIRE_PROTOCOL_VERSION, 19);
