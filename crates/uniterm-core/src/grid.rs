...
/// Where the cursor's and the former viewport top's logical lines land during
/// a reflow: which logical line each is, and that line's first physical row
/// and row count once it has been emitted.
struct ReflowAnchors {
    logical_index: usize,
    cursor_logical: usize,
    screen_logical: usize,
    cursor_span: (usize, usize),
    screen_span: (usize, usize),
}

impl Default for ReflowAnchors {
    fn default() -> Self {
        // Every reflow emits at least one logical line, so a span of one row
        // at the top is the fallback that can never index past the output.
        ReflowAnchors {
            logical_index: 0,
            cursor_logical: 0,
            screen_logical: 0,
...
impl ReflowAnchors {
    fn record(&mut self, span: (usize, usize)) {
        if self.logical_index == self.cursor_logical {
            self.cursor_span = span;
        }
        if self.logical_index == self.screen_logical {
            self.screen_span = span;
        }
...
impl Grid {
    ...
    /// Copy the live screen rows into a standalone grid of the same size with
    /// no scrollback. Grapheme, underline-colour, and hyperlink handles are
    /// re-interned, so the copy stays valid after this grid compacts an arena
    /// or swaps to the alternate screen. Copy-mode holds one while a text
    /// selection is in progress with `freeze-on-select` on, so an application
    /// that keeps painting cannot move the text under the pointer
    /// (`docs/04-multiplexer-core.md`).
    pub fn freeze_screen(&self) -> Grid {
        let mut frozen = Grid::new(self.width, self.height);
        frozen.set_scrollback_limit(0);
        for (y, line) in self.lines.iter().enumerate() {
            frozen.lines[y].wrapped = line.wrapped;
            for (x, &cell) in line.cells.iter().enumerate().take(self.width as usize) {
                let mut copy = cell;
                if cell.cluster != 0 {
                    match self.clusters.get(cell.cluster as usize) {
                        Some(text) => copy.cluster = frozen.intern(text),
                        None => {
                            copy.cluster = 0;
                            copy.ch = '\u{fffd}';
                        }
                ...
                if cell.underline_color != 0 {
                    copy.underline_color =
                        frozen.intern_underline_color(self.underline_color(cell.underline_color));
                }
                if cell.hyperlink != 0 {
                    copy.hyperlink = self
                        .hyperlink(cell.hyperlink)
                        .map_or(0, |uri| frozen.intern_hyperlink(uri));
                }
                frozen.lines[y].cells[x] = copy;
            }
        }
        frozen
    }
    ...
    pub fn resize_reflow(
        ...
        // Shrink vertically at the same width without touching history: the
        // rows leaving the top of the viewport become the newest history
        // lines, exactly where a full reflow at this width would put them,
        // and the cursor keeps its column and its pending wrap.
        if height < self.height && width == self.width {
            let removed = (self.height - height) as usize;
            let departing: Vec<Line> = self.lines.drain(..removed).collect();
            for mut line in departing {
            ...
            let cursor_y = (cursor.1 as usize).saturating_sub(removed) as u16;
            return (cursor_x.min(width - 1), cursor_y, cursor_pending);
        }

        self.reflow_width(width, height, blank, cursor, cursor_pending)
    }

    /// Reflow every retained line to a new width, streaming `Cell` to `Cell`.
    ///
    /// Old rows are consumed front to back and freed as soon as their glyph
    /// leads are copied into one reusable scratch buffer per logical line;
    /// new rows are cut from that buffer at the new width. Nothing here builds
    /// a persisted `StoredCell` or a `String`, and hard-newline rows keep only
    /// their content, so a reflow costs a copy of the retained cells rather
    /// than three String-per-cell snapshots of the whole history. Grapheme,
    /// hyperlink, and underline-colour handles stay valid because the grid's
    /// arenas are kept rather than rebuilt.
    fn reflow_width(
        &mut self,
        width: u16,
        height: u16,
        ...
        let total = old_scrollback.len() + old_lines.len();

        // Glyph leads of the logical line under construction, and the physical
        // row being cut from it. Both are reused for every line.
        let mut leads: Vec<Cell> = Vec::with_capacity(old_width as usize * 2);
        let mut row: Vec<Cell> = Vec::with_capacity(width as usize);
        let mut physical: VecDeque<Line> = VecDeque::with_capacity(total);
        // Sum of the leads' stored widths across the finished rows of the
        // current logical line: the cursor's offset into it is measured in
        // these cells.
        let mut current_width = 0usize;
        let mut cursor_offset = 0usize;
        let mut screen_offset = 0usize;
        let mut anchors = ReflowAnchors::default();

        for (line_idx, line) in old_scrollback.into_iter().chain(old_lines).enumerate() {
            if line_idx == old_cursor_line {
                anchors.cursor_logical = anchors.logical_index;
                cursor_offset = current_width + cursor.0 as usize;
            }
            if line_idx == old_screen_line {
                anchors.screen_logical = anchors.logical_index;
                screen_offset = current_width;
            }
            let before = leads.len();
            leads.extend(line.cells.iter().copied().filter(|c| !c.is_continuation()));
            if line.wrapped {
                // A soft-wrapped row spans the full width; a short one is
                // padded like its persisted form would be.
                let missing = (old_width as usize).saturating_sub(line.cells.len());
                leads.extend(std::iter::repeat_n(Cell::default(), missing));
            } else {
                while leads.len() > before && self.is_default_blank(leads[leads.len() - 1]) {
                    leads.pop();
                }
            }
            current_width += leads[before..]
                .iter()
                .map(|c| c.width.max(1) as usize)
                .sum::<usize>();
            let wrapped = line.wrapped;
            drop(line);
            if !wrapped {
                let span = self.emit_logical_line(&leads, width, &mut row, &mut physical);
                anchors.record(span);
                leads.clear();
                current_width = 0;
            }
        ...
        if anchors.logical_index == 0 {
            let span = self.emit_logical_line(&leads, width, &mut row, &mut physical);
            anchors.record(span);
        }

        // Map the cursor through its logical offset. A pending wrap whose
        // offset lands exactly on a row boundary belongs at the end of the row
        // before that boundary, still pending; anything else lands where the
        // offset falls and the pending state is consumed by the reflow.
        let w = width as usize;
        let (cursor_start, cursor_count) = anchors.cursor_span;
        let mut cursor_row = cursor_offset / w;
        let mut cursor_x = (cursor_offset % w) as u16;
        let mut mapped_pending = false;
        if cursor_pending && cursor_x == 0 && cursor_offset != 0 {
            cursor_row -= 1;
            cursor_x = width - 1;
            mapped_pending = true;
        }
        let mut cursor_global = cursor_start + cursor_row.min(cursor_count - 1);

        let (screen_first, screen_count) = anchors.screen_span;
        let reflowed_screen_start = screen_first + (screen_offset / w).min(screen_count - 1);
        let screen_start = if height > old_height {
            // Preserve the former viewport top across growth. If narrower
            // reflow puts the cursor or the last row with content below the
            // enlarged viewport, advance only far enough to keep them visible;
            // the rows that then fall off the bottom are blank.
            let mut start =
                reflowed_screen_start.max((cursor_global + 1).saturating_sub(height as usize));
            if physical.len() > start + height as usize {
                let content_end = physical
                    .iter()
                    .rposition(|line| line.wrapped || !line.cells.is_empty())
                    .map_or(0, |index| index + 1);
                let needed_end = content_end.max(cursor_global + 1);
                start = start.max(needed_end.saturating_sub(height as usize));
                physical.truncate(start + height as usize);
            }
            while physical.len() < start + height as usize {
                let id = self.alloc_line_id();
                physical.push_back(Line::blank(width, blank, id));
            }
            start
        } else {
            while physical.len() < height as usize {
                let id = self.alloc_line_id();
                physical.push_front(Line::blank(width, blank, id));
                cursor_global += 1;
            }
            physical.len() - height as usize
        };
        ...
        physical.drain(..history_start);
        let retained_screen_start = screen_start - history_start;

        self.lines = physical.split_off(retained_screen_start).into();
        self.scrollback = physical;
        // Visible rows are addressed by column, so restore the default cells
        // trimmed from their tails; history rows stay trimmed.
        for line in &mut self.lines {
            if line.cells.len() < w {
                let mut cells = std::mem::take(&mut line.cells).into_vec();
                cells.resize(w, Cell::default());
                line.cells = cells.into_boxed_slice();
            }
        }
        self.width = width;
        self.height = height;
    ...
    /// Cut one logical line's glyph leads into physical rows of `width`,
    /// appending them to `physical`. Returns the first row and the row count.
    /// A hard-newline row keeps only its content; wrapped rows are full width,
    /// with a column left empty by a wide glyph that did not fit holding a
    /// terminal-default cell. Cells are never restyled: what was empty stays
    /// empty in its own colours rather than taking the current erase colour.
    fn emit_logical_line(
        &mut self,
        leads: &[Cell],
        width: u16,
        row: &mut Vec<Cell>,
        physical: &mut VecDeque<Line>,
    ) -> (usize, usize) {
        let start = physical.len();
        row.clear();
        let mut x = 0u16;
        for &lead in leads {
            let glyph_width = self.glyph_width(lead).min(width);
            if x > 0 && x + glyph_width > width {
                row.resize(width as usize, Cell::default());
                let id = self.alloc_line_id();
                physical.push_back(Line {
                    cells: row.as_slice().into(),
                    wrapped: true,
                    id,
                });
                row.clear();
        ...
        row.clear();
        (start, physical.len() - start)
    }

    /// Natural display width of a glyph lead, from its text: one or two cells.
    /// A wide glyph squeezed into a one-column grid regains its width here.
    fn glyph_width(&self, cell: Cell) -> u16 {
        if cell.cluster != 0 {
            self.clusters
                .get(cell.cluster as usize)
                .map_or(1, |text| text.as_str().width().clamp(1, 2) as u16)
        } else {
            let mut bytes = [0u8; 4];
            cell.ch.encode_utf8(&mut bytes).width().clamp(1, 2) as u16
        }
    }

    /// A plain space in terminal-default style, whose absence at the end of a
    /// hard-newline line changes nothing visible.
    fn is_default_blank(&self, cell: Cell) -> bool {
        cell.ch == ' '
            && cell.cluster == 0
            ...
            && cell.hyperlink == 0
            && self.underline_color(cell.underline_color) == Color::Default
    }
...
mod tests {
    ...
    #[test]
    fn a_frozen_screen_survives_arena_compaction_and_later_output() {
        let mut grid = Grid::new(6, 2);
        let red = grid.intern_underline_color(Color::Rgb(255, 0, 0));
        let link = grid.intern_hyperlink("https://example.test/");
        let styled = Cell {
            fg: Color::Idx(3),
            underline_color: red,
            hyperlink: link,
        ...
        grid.set_grapheme(0, 0, "e\u{301}", styled, 1);
        grid.set_grapheme(1, 0, "\u{754c}", Cell::default(), 2);
        grid.set(3, 0, cell('x'));
        grid.set_row_wrapped(0, true);
        grid.set(0, 1, cell('y'));

        let frozen = grid.freeze_screen();
        assert_eq!((frozen.width(), frozen.height()), (6, 2));
        assert_eq!(frozen.total_lines(), 2);
        assert_eq!(frozen.scrollback_len(), 0);

        // Retire the source handles: overwrite the cells that used them, then
        // push enough distinct graphemes through one cell to compact the
        // arena, and scroll the live rows away entirely.
        grid.set(0, 0, cell('-'));
        for i in 0..5000u32 {
            let text = format!(
                "a{}{}",
                char::from_u32(0x300 + i % 112).unwrap(),
                char::from_u32(0x300 + i / 112).unwrap()
            );
            grid.set_grapheme(5, 1, &text, Cell::default(), 1);
        }
        grid.scroll_up(Cell::default());
        grid.scroll_up(Cell::default());
        assert_eq!(grid.line_cell(2, 3), Cell::default());

        let lead = frozen.line_cell(0, 0);
        assert_eq!(frozen.cell_text_owned(lead), "e\u{301}");
        assert_eq!(lead.fg, Color::Idx(3));
        assert_eq!(
            frozen.underline_color(lead.underline_color),
            Color::Rgb(255, 0, 0)
        );
        assert_eq!(
            frozen.hyperlink(lead.hyperlink),
            Some("https://example.test/")
        );
        let wide = frozen.line_cell(0, 1);
        assert_eq!((wide.ch, wide.width), ('\u{754c}', 2));
        assert!(frozen.line_cell(0, 2).is_continuation());
        assert_eq!(frozen.line_cell(0, 3).ch, 'x');
        assert!(frozen.line_wrapped(0));
        assert_eq!(frozen.line_cell(1, 0).ch, 'y');
        assert!(!frozen.line_wrapped(1));
    }
    ...
    #[test]
    fn shrinking_height_at_equal_width_moves_top_rows_into_history() {
        let mut grid = Grid::new(5, 4);
        for (y, ch) in ['a', 'b', 'c', 'd'].into_iter().enumerate() {
            grid.set(0, y as u16, cell(ch));
        }
        grid.set_row_wrapped(0, true);
        let cursor = grid.resize_reflow(5, 2, Cell::default(), (1, 3), false);
        assert_eq!(cursor, (1, 1, false));
        assert_eq!(grid.height(), 2);
        assert_eq!(grid.scrollback_len(), 2);
        let rows: Vec<char> = (0..grid.total_lines())
            .map(|line| grid.line_cell(line, 0).ch)
            .collect();
        assert_eq!(rows, vec!['a', 'b', 'c', 'd']);
        assert!(grid.line_wrapped(0));
        assert_eq!(grid.retained_cell_count(), 5 + 1 + 5 + 5);
        assert!(grid.is_dirty());

        // A cursor on a departing row lands on the new top row, and a pending
        // wrap survives with its column.
        let mut grid = Grid::new(5, 3);
        grid.set(4, 0, cell('x'));
        let cursor = grid.resize_reflow(5, 1, Cell::default(), (5, 0), true);
        assert_eq!(cursor, (4, 0, true));
        assert_eq!(grid.scrollback_len(), 2);

        // The alternate screen keeps no history, so departing rows vanish.
        let mut grid = Grid::new(5, 3);
        grid.set_scrollback_limit(0);
        grid.resize_reflow(5, 2, Cell::default(), (0, 2), false);
        assert_eq!(grid.scrollback_len(), 0);
    ...
    /// The reflow that shipped before the streaming rewrite, kept as the
    /// oracle for `streaming_reflow_matches_the_reference_on_random_grids`.
    /// It is the removed implementation verbatim, materializing every line as
    /// `StoredCell`s and rebuilding the grid, except for two marked
    /// corrections of behaviour the rewrite deliberately changed. Line ids and
    /// hyperlinks are not comparable: this path restarts ids and drops links.
    impl Grid {
        fn resize_reflow_reference(
            &mut self,
            width: u16,
            height: u16,
            blank: Cell,
            ...
            // Correction 3: the shipped code seeded every rewrapped row with
            // the current erase colour, restyling each trailing empty cell of
            // history and screen alike on every resize. Rows now keep their
            // cells; only whole padding rows take the erase colour.
            let default_saved = StoredCell {
                text: " ".into(),
                fg: Color::Default,
                bg: Color::Default,
                attrs: Attrs::NONE,
            ...
            for glyphs in &logical {
                ...
                let mut row = stored_blank_line(width, &default_saved);
                let mut x = 0u16;
                for glyph in glyphs {
                    ...
                    if x > 0 && x + glyph_width > width {
                        row.wrapped = true;
                        physical.push(row);
                        row = stored_blank_line(width, &default_saved);
                        x = 0;
                    }
            ...
            // Correction 1: the shipped code clamped the row to the logical line
            // before stepping back for a pending wrap, so a cursor pending at the
            // very end of its line landed one row too high.
            let mut cursor_row_in_logical = cursor_offset / width as usize;
            ...
            let cursor_row_in_logical = cursor_row_in_logical.min(logical_count - 1);
            ...
            let screen_start = if height > self.height {
                ...
                let mut start = reflowed_screen_start.max(cursor_start);
                // Correction 2: the shipped code kept every reflowed row of the
                // old viewport as a visible line even when there were more than
                // `height` of them, leaving rows nothing could ever paint.
                if physical.len() > start + height as usize {
                    let content_end = physical
                        .iter()
                        .rposition(|row| {
                            row.wrapped || !row.cells.iter().all(stored_cell_is_default_blank)
                        })
                        .map_or(0, |index| index + 1);
                    let needed_end = content_end.max(cursor_global + 1);
                    start = start.max(needed_end.saturating_sub(height as usize));
    ...
    fn random_style(rng: &mut Rng) -> Cell {
        let mut cell = Cell::default();
        match rng.below(6) {
            0 => cell.fg = Color::Idx(rng.below(256) as u8),
            1 => cell.bg = Color::Rgb(rng.below(256) as u8, 0, 7),
            2 => cell.attrs = Attrs::BOLD,
            3 => cell.attrs = Attrs::ITALIC,
            _ => {}
        }
        cell
    }

    /// Fill row `y` with random glyphs: ASCII, spaces, wide CJK, combining
    /// sequences, underline colours, and short lines. Returns whether the row
    /// was filled to its last column. Glyphs go through `set_grapheme`, as
    /// the emulator's own writes do, so overwriting half of a wide glyph
    /// clears its other half instead of leaving an orphan continuation.
    fn random_row(grid: &mut Grid, rng: &mut Rng, y: u16) -> bool {
        ...
        while x < width {
            ...
            match rng.below(12) {
                0..=5 => {
                    ...
                    grid.set_grapheme(x, y, ch.encode_utf8(&mut [0; 4]), style, 1);
                    x += 1;
                }
                6 => x += 1,
                7 if x + 1 < width => {
                    grid.set_grapheme(x, y, "界", style, 2);
                    x += 2;
                }
                8 => {
                    grid.set_grapheme(x, y, "e\u{301}", style, 1);
                    x += 1;
                }
                9 => {
                    let handle = grid.intern_underline_color(Color::Idx(rng.below(16) as u8));
                    let styled = Cell {
                        underline_color: handle,
                        ..style
                    };
                    grid.set_grapheme(x, y, "u", styled, 1);
                    x += 1;
                }
                10 => return false,
                _ => x += 1,
    ...
    fn random_grid(rng: &mut Rng) -> (Grid, Cell, (u16, u16), bool) {
        let width = 1 + rng.below(20) as u16;
        let height = 1 + rng.below(6) as u16;
        let mut grid = Grid::new(width, height);
        grid.set_scrollback_limit([0usize, 2, 5, 10_000][rng.below(4) as usize]);
        let blank = if rng.chance(25) {
            Cell {
                bg: Color::Rgb(0, 0, 40),
                ..Cell::default()
            }
        ...
        for _ in 0..rng.below(12) {
            let full = random_row(&mut grid, rng, height - 1);
            if full && rng.chance(50) {
                grid.set_row_wrapped(height - 1, true);
            }
            grid.scroll_up(blank);
        }
        for y in 0..height {
            let full = random_row(&mut grid, rng, y);
            // The emulator only marks a row wrapped once the cursor has moved
            // to the row below it, so the last row is never wrapped.
            if full && y + 1 < height && rng.chance(50) {
                grid.set_row_wrapped(y, true);
            }
        }
        let pending = rng.chance(20);
        let cy = rng.below(height as u64) as u16;
        let cx = if pending {
            width
        } else {
            rng.below(width as u64) as u16
        };
        (grid, blank, (cx, cy), pending)
    }

    type CellView = (String, u8, bool, Color, Color, Attrs, Color);

    /// Everything a reader can observe about a line: wrap flag plus each
    /// cell's text, width, continuation, colours, attributes, and underline
    /// colour resolved through the grid's own arena.
    fn visible(grid: &Grid) -> Vec<(bool, Vec<CellView>)> {
        (0..grid.total_lines())
            .map(|line| {
                let cells = (0..grid.width())
                    ...
                    .collect();
                (grid.line_wrapped(line), cells)
            })
            .collect()
    }

    #[test]
    fn streaming_reflow_matches_the_reference_on_random_grids() {
        let mut rng = Rng(0x9E37_79B9_7F4A_7C15);
        for case in 0..3000 {
            let seed = rng.next();
            let mut case_rng = Rng(seed);
            let (mut grid, blank, mut cursor, mut pending) = random_grid(&mut case_rng);
            let mut reference = grid.clone();
            for step in 0..1 + case_rng.below(3) {
                // Height-only changes take the fast paths, which keep rows
                // verbatim; the old rewrap is not idempotent there (it drops a
                // soft-wrapped line's trailing blank row), so those are
                // specified by `height_only_resizes_keep_every_row_verbatim`.
                let width = loop {
                    let width = 1 + case_rng.below(24) as u16;
                    if width != grid.width() {
                        break width;
                    }
                };
                let height = 1 + case_rng.below(8) as u16;
                let expected =
                    reference.resize_reflow_reference(width, height, blank, cursor, pending);
                let actual = grid.resize_reflow(width, height, blank, cursor, pending);
                let context = format!(
                    "case {case} seed {seed:#x} step {step}: resize to {width}x{height}, cursor {cursor:?} pending {pending}"
                );
                assert_eq!(actual, expected, "cursor differs: {context}");
                assert_eq!(grid.width(), reference.width(), "{context}");
                assert_eq!(grid.height(), reference.height(), "{context}");
                assert_eq!(
                    grid.scrollback_len(),
                    reference.scrollback_len(),
                    "history differs: {context}"
                );
                let (actual_lines, expected_lines) = (visible(&grid), visible(&reference));
                if actual_lines != expected_lines {
                    ...
                    panic!(
                        "content differs at line {first} of {} vs {}: {context}\n  new: {:?}\n  ref: {:?}",
                        actual_lines.len(),
                        expected_lines.len(),
                ...
                assert_eq!(grid.lines.len(), height as usize, "{context}");
                assert!(
                    grid.lines
                        .iter()
                        .all(|line| line.cells.len() == width as usize),
                    "a visible row is not full width: {context}"
                );
                assert!(
                    grid.retained_cell_count() <= reference.retained_cell_count(),
                    "{context}"
                );
                cursor = (actual.0 + u16::from(actual.2), actual.1);
                pending = actual.2;
            }
        }
    }
    ...
    #[test]
    fn height_only_resizes_keep_every_row_verbatim() {
        let mut rng = Rng(0x1234_5678_9ABC_DEF1);
        for case in 0..2000 {
            let seed = rng.next();
            let mut case_rng = Rng(seed);
            let (mut grid, blank, cursor, pending) = random_grid(&mut case_rng);
            let width = grid.width();
            let old_height = grid.height();
            let before = visible(&grid);
            let height = 1 + case_rng.below(8) as u16;
            if height == old_height {
                // Same size: the terminal never calls into the grid.
                continue;
            }
            let old_history = before.len() - old_height as usize;
            let mapped = grid.resize_reflow(width, height, blank, cursor, pending);
            let after = visible(&grid);
            let context =
                format!("case {case} seed {seed:#x}: {width}x{old_height} to {width}x{height}");
            if height < old_height {
                // Rows leave the top of the viewport into history; only the
                // history cap can drop any of them, and only the oldest.
                let removed = (old_height - height) as usize;
                assert_eq!(
                    grid.scrollback_len(),
                    (old_history + removed).min(grid.scrollback_limit),
                    "{context}"
                );
                let kept = grid.scrollback_len() + height as usize;
                assert_eq!(after.len(), kept, "{context}");
                assert_eq!(
                    after[..],
                    before[before.len() - kept..],
                    "rows changed: {context}"
                );
                let cursor_x = if pending { cursor.0 - 1 } else { cursor.0 };
                assert_eq!(
                    mapped,
                    (
                        cursor_x,
                        (cursor.1 as usize).saturating_sub(removed) as u16,
                        pending
                    ),
                    "{context}"
                );
            } else {
                // Growth appends blank rows below the live screen.
                assert_eq!(
                    after.len(),
                    before.len() + (height - old_height) as usize,
                    "{context}"
                );
                assert_eq!(after[..before.len()], before[..], "rows changed: {context}");
                let cursor_x = if pending { cursor.0 - 1 } else { cursor.0 };
                assert_eq!(mapped, (cursor_x, cursor.1, pending), "{context}");
            }
            assert_eq!(grid.lines.len(), height as usize, "{context}");
        }
    }
    ...
    #[test]
    fn a_pending_cursor_at_the_end_of_its_line_stays_on_its_last_row() {
        let mut grid = Grid::new(6, 1);
        for (x, ch) in "abcdef".chars().enumerate() {
            grid.set(x as u16, 0, cell(ch));
        }
        let cursor = grid.resize_reflow(3, 3, Cell::default(), (6, 0), true);
        assert_eq!(cursor, (2, 1, true));
        let rows: Vec<String> = (0..grid.total_lines())
            .map(|line| (0..3).map(|x| grid.line_cell(line, x).ch).collect())
            .collect();
        assert_eq!(rows, vec!["abc", "def", "   "]);
        assert!(grid.line_wrapped(0));
        assert!(!grid.line_wrapped(1));
    }

    #[test]
    fn growing_while_narrowing_keeps_every_row_reachable() {
        let mut grid = Grid::new(8, 2);
        for y in 0..2 {
            for (x, ch) in "abcdefgh".chars().enumerate() {
                grid.set(x as u16, y, cell(ch));
            }
        }
        let cursor = grid.resize_reflow(4, 3, Cell::default(), (0, 1), false);
        assert_eq!(grid.lines.len(), 3);
        assert_eq!(grid.scrollback_len(), 1);
        let rows: Vec<String> = (0..grid.total_lines())
            .map(|line| (0..4).map(|x| grid.line_cell(line, x).ch).collect())
            .collect();
        assert_eq!(rows, vec!["abcd", "efgh", "abcd", "efgh"]);
        assert_eq!(cursor, (0, 1, false));
        assert_eq!(grid.get(0, 1).ch, 'a');
    }

    #[test]
    fn hyperlinks_and_grapheme_handles_survive_a_reflow() {
        let mut grid = Grid::new(6, 1);
        let link = grid.intern_hyperlink("https://example.test/a");
        grid.set(
            0,
            0,
            Cell {
        ...
        grid.set_grapheme(1, 0, "e\u{301}", cell('e'), 1);
        grid.set_grapheme(2, 0, "界", cell('界'), 2);
        grid.resize_reflow(2, 3, Cell::default(), (4, 0), false);
        assert_eq!(
            grid.hyperlink(grid.get(0, 0).hyperlink),
            Some("https://example.test/a")
        );
        assert_eq!(grid.cell_text_owned(grid.get(1, 0)), "e\u{301}");
        assert_eq!(grid.cell_text_owned(grid.get(0, 1)), "界");
        assert!(grid.get(1, 1).is_continuation());
        assert!(grid.line_wrapped(0));
    }

    #[test]
    fn reflow_retains_only_content_for_hard_newline_history() {
        ...
        grid.resize_reflow(60, 3, Cell::default(), (0, 0), false);
        assert_eq!(grid.scrollback_len(), 1000);
        assert!(
            grid.retained_cell_count() <= 1000 * 2 + 3 * 60,
            "history was padded to full width: {} cells",
            grid.retained_cell_count()
        );
        assert_eq!(grid.line_cell(0, 1).ch, 'i');
        assert_eq!(grid.line_cell(0, 2), Cell::default());
    }
}
