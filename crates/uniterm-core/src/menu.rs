...
pub enum MenuAction {
    ...
    NextTab,
    PrevTab,
    /// Reorder the selected Tab without changing its Pane selection.
    MoveTabLeft,
    /// Reorder the selected Tab without changing its Pane selection.
    MoveTabRight,
    /// Open the day timeline without changing the remembered Project.
    Today,
    NewTask,
    Observatory,
    Tasks,
    ...
    Menu {
        ...
        items: &[
            ...
            MenuItem {
                label: "Move tab left",
                key: "<",
                action: MenuAction::MoveTabLeft,
            },
            MenuItem {
                label: "Move tab right",
                key: ">",
                action: MenuAction::MoveTabRight,
            },
        ],
    },
    Menu {
...
mod tests {
    ...
    fn workspace_menu_has_requested_groups_and_actions() {
        ...
        assert_eq!(workspace.separators_before, &[1, 3, 5, 8, 9]);
        ...
        assert_eq!(workspace.items[5].action, MenuAction::Sidebar);
        assert_eq!(workspace.items[1].key, "A");
        assert_eq!(workspace.items[5].key, "b");
        assert_eq!(workspace.items[6].action, MenuAction::Observatory);
        assert_eq!(workspace.items[7].action, MenuAction::Settings);
        assert_eq!(workspace.items[8].action, MenuAction::About);
        assert_eq!(workspace.items[8].key, "");
        assert_eq!(workspace.items[10].key, "Q");
        assert_eq!(
