...
fn envelope_frame(
    ...
    let event = if metadata_only {
        use crate::eventlog::LogEvent;
        match &envelope.event {
            LogEvent::AgentBound { .. } | LogEvent::AgentStatus { .. }
            | LogEvent::AgentUnbound { .. } | LogEvent::ProjectSelected { .. }
            | LogEvent::TimelineBoundary { .. } => serde_json::to_value(&envelope.event),
            LogEvent::AgentSessionObserved { pane, provider, session_id, .. } => Ok(serde_json::json!({
                "AgentSessionObserved": { "pane": pane, "provider": provider, "session_id": session_id }
            })),
            _ => Ok(serde_json::json!({"redacted": true})),
        }
    } else { serde_json::to_value(&envelope.event) }
...
async fn stream_subscription(start: SubscriptionStart) {
    ...
    while let Some(envelope) = history_rx.recv().await {
        let Ok(frame) = envelope_frame(subscription, envelope, metadata_only) else {
...
async fn agent_main(
    ...
    if let Some(path) = control_path {
        ...
        manager_control = crate::control_api::Listener::bind_scoped(
            manager_path,
            control_tx.clone(),
            control_generation,
            true,
    ...
    let mut read_only_connections = HashSet::new();
    ...
    let git_watchers = crate::git_status::GitWatchService::new(tx.clone(), waker.clone());
    ...
    let mut process_cache: HashMap<(PaneId, i32), crate::providers::Match> = HashMap::new();
    let mut persistence = persistence::Persistence::default();
    let timeline = Arc::new(std::sync::Mutex::new(crate::timeline::Index::default()));
    let mut timeline_watch = false;
    ...
    loop {
        ...
        let msg = match input {
            ...
            RuntimeInput::Control(crate::control_api::Inbound::Connected {
                ...
                if read_only {
                    read_only_connections.insert(connection);
                }
                continue;
            }
            RuntimeInput::Control(crate::control_api::Inbound::Request {
                ...
                if read_only_connections.contains(&connection)
                    && !matches!(
                        request.command,
                        ControlCommand::TimelineQuery { .. }
                            | ControlCommand::AgentList
                            | ControlCommand::Capabilities
                            | ControlCommand::Subscribe { .. }
                    )
                {
                    if let Some(output) = control_outputs.get(&connection) {
                        let response = uniterm_proto::ControlResponse::error(request.id, "read_only", "The manager endpoint permits only timeline, fleet, and metadata subscriptions");
                        crate::control_api::send_bounded(output, ControlFrame::Response(response));
                    }
                    continue;
                }
                let pending_limit =
            ...
            RuntimeInput::Control(crate::control_api::Inbound::Disconnected {
                ...
                control_outputs.remove(&connection);
                read_only_connections.remove(&connection);
                subscribe_after.retain(|(tracked, _), _| *tracked != connection);
                subscriptions.remove(&connection);
                continue;
            }
        ...
        let reply = match msg {
            CoreToAgent::ControlRename { workspace, path } => {
                ...
                subscriptions.clear();
                drop(control.take());
                drop(manager_control.take());
                read_only_connections.clear();
            ...
            CoreToAgent::ControlResponse {
                ...
                if read_only_connections.contains(&connection) {
                    match response.result.as_mut() {
                        Some(ControlResult::Timeline { day }) => {
                            let mut index = 0;
                            day.times.retain(|_| {
                                let keep = day.entries.get(index).is_some_and(|entry| {
                                    !matches!(
                                        entry.kind,
                        ...
                        Some(ControlResult::Fleet { entries }) => {
                            for entry in entries {
                                entry.evidence.clear();
                            }
                        }
                        Some(ControlResult::Capabilities { capabilities, .. }) => {
                            *capabilities = vec![
                                "timeline.query".into(),
                                "agent.list".into(),
                                "manager.read_only".into(),
                                "subscribe".into(),
                            ]
                        }
                        _ => {}
                    ...
                    if !crate::control_api::send_bounded(&output, ControlFrame::Response(response))
                    {
                        ...
                        if let Some(workspace) = control_workspace.clone() {
                            ...
                            tokio::spawn(stream_subscription(SubscriptionStart {
                                ...
                                metadata_only: read_only_connections.contains(&connection),
                            }));
                        }
                    }
                }
            ...
            CoreToAgent::EditorLaunchExternal {
                project,
                path,
                command,
                cwd,
                ...
                let waker = waker.clone();
                tokio::spawn(async move {
                    let error = launch_external_editor(&command, &path, &cwd).await.err();
                    if tx
                        .send(AgentToCore::EditorExternalFinished { project, error })
                        .is_ok()
                    {
                        let _ = waker.wake();
                    }
            ...
            CoreToAgent::EditorOpen {
                ...
                tokio::task::spawn_blocking(move || AgentToCore::EditorResolved {
                    ...
                    reuse_tab: editor_reuses_tab(&command),
                    command,
                })
                .await
            ...
            CoreToAgent::EventAppend { name, line } => {
                ...
                let changed = appended
                    .as_ref()
                    .filter(|event| timeline_watch && crate::timeline::changes_day(&event.event))
                ...
                reply.or_else(|| changed.map(|sequence| AgentToCore::TimelineChanged { sequence }))
            }
            CoreToAgent::TimelineResume {
                name,
                sequence,
                requester,
            } => {
                ...
                tokio::spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                        crate::timeline::resume(&name, sequence).map_err(|e| e.to_string())
                    })
                    .await
                    .unwrap_or_else(|e| Err(e.to_string()));
                    if tx
                        .send(AgentToCore::TimelineResumeReady { requester, result })
                        .is_ok()
                    {
                        let _ = waker.wake();
                    }
            ...
            CoreToAgent::TimelineWatch { enabled } => {
                timeline_watch = enabled;
                None
            }
            CoreToAgent::TimelineQuery {
                name,
                date,
                filter,
                before,
                ...
                tokio::spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                        timeline
                            .lock()
                            .map_err(|_| "Timeline index worker failed".to_owned())?
                            .query(&name, &date, &filter, before, through)
                            .map_err(|e| e.to_string())
                    })
                    .await
                    .unwrap_or_else(|e| Err(e.to_string()));
                    if tx
                        .send(AgentToCore::TimelineLoaded { requester, result })
                        .is_ok()
                    {
                        let _ = waker.wake();
                    }
            ...
            CoreToAgent::PaneEvidence {
                ...
                let process = if bound_agent.is_some() {
                    ...
                    if process_changed || !process_cache.contains_key(&key) {
                        ...
                        process_cache.retain(|(tracked, _), _| *tracked != pane);
                        // A launcher can exec an agent without changing its PID.
                        // Retry misses on the next changed screen/title, never
                        // on an idle timer. Only confirmed identities are cached.
                        if let Some(found) = &found {
                            process_cache.insert(key, found.clone());
                        }
                        found
                    } else {
                        process_cache.get(&key).cloned()
                    }
                } else {
                    None
                };
            ...
            CoreToAgent::DevServerStop {
                ...
                tokio::spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                        crate::dev_server::stop_listener(session, port)
                    })
                    .await
                    ...
                    let error = match result {
                        ...
                        Ok(())
                            if crate::dev_server::probe_port(port).await
                                == crate::dev_server::PortProbe::Refused =>
                        {
                            None
                        }
                        Ok(()) => Some(
                            "The port is still listening or could not be verified closed".into(),
                        ),
                    };
                    if tx
                        .send(AgentToCore::DevServerStopFinished {
                            pane,
                            port,
                            detected,
                            error,
            ...
            CoreToAgent::GitChangeWatchSet { project, root } => {
                git_watchers.set(project, root);
                None
            }
        };
...
/// Launch a GUI editor without allocating a PTY or waiting in the command
/// dispatcher. Tokio reaps this exact child on exit, including launchers that
/// keep running for the lifetime of their application.
async fn launch_external_editor(command: &str, path: &str, cwd: &str) -> Result<(), String> {
    let words =
        shell_words::split(command).map_err(|error| format!("Invalid editor command: {error}"))?;
    let program = words
        .first()
...
fn editor_reuses_tab(command: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn editor_tab_reuse_excludes_external_launchers() {
        for command in [
            "vi",
            "nvim --clean",
            "nano",
            ...
            assert!(editor_reuses_tab(command), "{command}");
        }
        for command in [
            "code --wait",
