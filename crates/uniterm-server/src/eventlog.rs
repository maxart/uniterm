...
impl StructuralProjection {
    ...
    pub fn from_snapshot<C>(snapshot: &crate::persist::Snapshot<C>) -> Self {
        Self {
...
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
pub enum LogEvent {
    /// A deliberate breadcrumb, separate from automatically observed activity.
    TimelineNote {
        context: uniterm_core::timeline::TimelineContext,
        text: String,
    },
    /// Prompt explicitly submitted through the protected retention command.
    TimelinePrompt {
        context: uniterm_core::timeline::TimelineContext,
        text: String,
    },
    /// A new invocation explicitly continues an earlier history entry.
    TimelineResumed {
        predecessor: u64,
        pane: u64,
    },
    /// Recovery does not imply that an old process or agent session survived.
    TimelineBoundary {
        reason: String,
    },
...
impl EventLog {
    ...
    pub fn record(&mut self, event: LogEvent) -> Option<(String, String)> {
        ...
        if self.last.as_ref() == Some(&event)
            && !matches!(
                event,
                LogEvent::TimelineNote { .. } | LogEvent::TimelinePrompt { .. }
            )
        {
            return None;
        }
...
fn visit_range(
    ...
    for line in std::io::BufReader::new(file).lines() {
        let line = line?;
        let line = crate::privacy::decode_line(&line)?;
        synthetic = synthetic.saturating_add(1);
        let envelope = match serde_json::from_str::<EventEnvelope>(&line) {
            Ok(envelope) if (1..=EVENT_VERSION).contains(&envelope.version) => envelope,
            Ok(envelope) => {
...
fn decode_record_strict(line: &str) -> Result<DecodedRecord, RecordDecodeError> {
    let decoded = crate::privacy::decode_line(line).map_err(|_| RecordDecodeError::Malformed)?;
    let line = decoded.as_ref();
    if let Ok(envelope) = serde_json::from_str::<EventEnvelope>(line) {
        if (1..=EVENT_VERSION).contains(&envelope.version) {
...
fn repair_consistent_prefix_path(
    ...
    let keep = consistent_prefix_len(std::io::BufReader::new(source), expected_workspace)?;
    if keep as u64 == length {
        return Ok(None);
    }
    let suffix = SystemTime::now()
        .duration_since(UNIX_EPOCH)
    ...
    let mut prefix = std::fs::File::open(path)?.take(keep as u64);
    std::io::copy(&mut prefix, &mut file)?;
    file.sync_all()?;
    std::fs::rename(&tmp, path)?;
    Ok(Some(RepairReport {
        backup,
        discarded_bytes: (length - keep as u64) as usize,
    }))
}

fn consistent_prefix_len(
    ...
    // Reuse one record buffer: large histories must not require an equally
    // large allocation just to verify their consistent prefix.
    ...
    loop {
        ...
        let line = match std::str::from_utf8(&bytes[..count - 1]) {
            Ok(line) => line,
            Err(_) => break,
        };
...
fn recover_snapshot_reader_scoped(
    ...
    for_each_record(reader, workspace, |record| {
        ...
        match record.event {
            LogEvent::WorkspaceProjected { state } => latest = Some((sequence, state)),
            event @ (LogEvent::AgentBound { .. }
            | LogEvent::AgentSessionObserved { .. }
            | LogEvent::AgentUnbound { .. }
            | LogEvent::PaneLaunchProfileCleared { .. }) => {
                if latest.is_none() {
                    latest = checkpoint.as_ref().map(|snapshot| {
                        (
                            checkpoint_sequence,
                            StructuralProjection::from_snapshot(snapshot),
                        )
                    });
                }
                let Some((cursor, state)) = latest.as_mut() else {
                    return;
                };
                let pane_id = match &event {
                    LogEvent::AgentBound { pane, .. }
                    | LogEvent::AgentSessionObserved { pane, .. }
                    | LogEvent::AgentUnbound { pane }
                    | LogEvent::PaneLaunchProfileCleared { pane } => *pane,
                    _ => unreachable!(),
                };
                let Some(pane) = state
                    .windows
                ...
                match event {
                    LogEvent::AgentBound { agent, .. } => {
                        pane.agent_launch = Some(StructuralAgentLaunch {
                            provider: agent,
                            session_id: None,
                    ...
                    LogEvent::AgentSessionObserved {
                        ...
                        let profile =
                            pane.agent_launch
                                .get_or_insert_with(|| StructuralAgentLaunch {
                        ...
                        if profile.provider == provider {
                            ...
                            if !resume_command.is_empty() {
                                profile.resume_command = resume_command;
                            }
                        }
                    }
                    LogEvent::AgentUnbound { .. } => {
                        pane.agent_launch = None;
                        pane.launch_args.clear();
                    }
                    LogEvent::PaneLaunchProfileCleared { .. } => pane.launch_args.clear(),
                    _ => unreachable!(),
                }
                *cursor = sequence;
            }
...
pub fn append_line(name: &str, line: &str) -> std::io::Result<()> {
    let path = EventLog::path(name);
    crate::persist::open_private_append(&path)?
        .write_all(crate::privacy::encode_line(name, line)?.as_bytes())
}
...
mod tests {
    ...
    #[test]
    fn repair_streams_across_small_buffers_and_propagates_read_errors() {
        let input = format!(
            "{}{}",
            envelope_line(41, LogEvent::PaneSpawned { pane: 1 }),
            envelope_line(42, LogEvent::PaneClosed { pane: 1 })
        );
        let reader = std::io::BufReader::with_capacity(7, input.as_bytes());
        assert_eq!(
            consistent_prefix_len(reader, "recover").unwrap(),
            input.len()
        );
        ...
        use std::io::Read as _;
        let reader = std::io::BufReader::new(input.as_bytes().chain(FailedRead));
        let error = consistent_prefix_len(reader, "recover").unwrap_err();
