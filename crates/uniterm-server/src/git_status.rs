...
/// Coalesces desired subscriptions while Git discovery runs independently of
/// editor requests and other runtime commands. The task sleeps until changed.
pub(crate) struct GitWatchService {
    desired: tokio::sync::watch::Sender<HashMap<ProjectId, String>>,
    task: tokio::task::JoinHandle<()>,
}

impl GitWatchService {
    pub(crate) fn new(tx: Sender<AgentToCore>, waker: Arc<Waker>) -> Self {
        let (desired, mut changes) =
            tokio::sync::watch::channel(HashMap::<ProjectId, String>::new());
        let task = tokio::spawn(async move {
            let mut manager = GitWatchManager::new();
            while changes.changed().await.is_ok() {
                let wanted = changes.borrow_and_update().clone();
                let removed: Vec<_> = manager
                    .projects
                    .keys()
                    .copied()
                    .filter(|project| !wanted.contains_key(project))
                    .collect();
                for project in removed {
                    manager.remove_project(project);
                }
                for (project, root) in wanted {
                    let reply = manager
                        .set(project, Some(root), tx.clone(), waker.clone())
                        .await;
    ...
    pub(crate) fn set(&self, project: ProjectId, root: Option<String>) {
        self.desired.send_if_modified(|desired| match root {
            Some(root) if desired.get(&project) != Some(&root) => {
                desired.insert(project, root);
                true
            }
            None => desired.remove(&project).is_some(),
            Some(_) => false,
        });
    }
}

impl Drop for GitWatchService {
    fn drop(&mut self) {
        self.task.abort();
    }
}
...
pub(crate) struct GitWatchManager {
    ...
    project_roots: HashMap<ProjectId, String>,
...
impl GitWatchManager {
    ...
    pub(crate) async fn set(
        ...
        if self.project_roots.get(&project) == Some(&root) {
        ...
        let discovered = tokio::task::spawn_blocking(move || discover_repository(&discover_root))
            .await
            .ok()
            .flatten();

        let Some(repository) = discovered else {
        ...
        let stats = if let Some(watch) = self.repositories.get(&repository) {
            ...
            let scan_root = repository.clone();
            let computed = tokio::task::spawn_blocking(move || compute_change_stats(&scan_root))
                .await
                .ok()
                .flatten();
            if let Some(computed) = &computed {
                if let Ok(watch) =
                    RepositoryWatch::new(repository.clone(), project, computed.clone(), tx, waker)
                {
        ...
        if stats.is_some() {
            self.project_roots.insert(project, root);
            self.projects.insert(project, repository);
        }

        AgentToCore::GitChangeStats { project, stats }
    }
    ...
    fn remove_project(&mut self, project: ProjectId) {
        self.project_roots.remove(&project);
    ...
    fn new(
        ...
        let mut watcher =
            notify::recommended_watcher(move |event: notify::Result<notify::Event>| {
                ...
                // and ignoring metadata-only ones does the same where reads
                // surface as attribute changes (kqueue reports git touching
                // objects and the index that way). Content changes always
                // arrive as data events alongside; only a bare permission
                // change goes unseen until the next content event.
                if matches!(
                    event.kind,
                ...
                handle.spawn(async move {
                    loop {
                        ...
                        .ok()
                        .flatten();
                        // Publish what was computed even if more events
                        // landed meanwhile: it is newer than what clients
                        // show, and the loop below recomputes for the rest.
                        // Discarding it let a scan that provokes its own
                        // events never report at all.
                        let changed = computed
                            .as_ref()
                            .is_some_and(|computed| *lock(&summary) != *computed);
                        if let Some(computed) = computed.filter(|_| changed) {
...
mod tests {
    ...
    async fn watcher_recomputes_after_a_debounced_change() {
        ...
        // Event-driven, so this returns as soon as the watcher fires; the
        // bound is generous because macOS file events can lag by seconds.
        let update =
            tokio::task::spawn_blocking(move || rx.recv_timeout(Duration::from_secs(30)).unwrap())
                .await
                .unwrap();
        assert!(matches!(
            update,
    ...
    async fn projects_in_one_repository_share_one_watcher() {
        ...
        manager
            ...
            .await;
        // Emulate a newer event-backed summary. Both a repeated subscription
        // and a nested Project must reuse it instead of rescanning the repo.
        let canonical = std::fs::canonicalize(&root).unwrap();
        let expected = GitChangeStats {
            files_changed: 17,
            insertions: 23,
            deletions: 4,
            untracked: 2,
        };
        *lock(&manager.repositories[&canonical].summary) = expected.clone();
        let repeated = manager
            ...
            .await;
        assert!(
            matches!(repeated, AgentToCore::GitChangeStats { stats: Some(stats), .. } if stats == expected)
        );
        let nested_reply = manager
            .set(
                ProjectId(2),
            ...
            .await;
        assert!(
            matches!(nested_reply, AgentToCore::GitChangeStats { stats: Some(stats), .. } if stats == expected)
        );
