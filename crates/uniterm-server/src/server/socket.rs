...
pub fn run_server(sock_path: &Path, program: &str, args: &[&str]) -> std::io::Result<()> {
    run_server_with_ready(sock_path, program, args, || Ok(()))
}

/// Notify the launcher only after recovery has completed, so large histories
/// can take as long as needed without the launcher killing a healthy server.
/// The callback runs once before entering the event loop and adds no idle work.
