        ...
        match msg {
            ...
            ClientMessage::Input(bytes) => {
                ...
                if self.context_menu.is_some() {
                    self.handle_context_input(reg, token, &bytes);
                } else if self.today.is_some() {
                    self.today_input(reg, &bytes);
    ...
    pub(super) fn handle_command(&mut self, reg: &Registry, cmd: Command) {
        ...
        match cmd {
            ...
            Command::Split(axis) => {
                ...
                self.split_pane(reg, pane, axis, false);
            }
            Command::Focus(fd) => {
                let dir = match fd {
            ...
            Command::NewWindow => {
                self.create_tab(reg, self.active_project, false);
            }
            Command::NextWindow => {
            ...
            Command::KillWindow => {
                self.close_tab(reg, self.active_window);
                if !self.running {
                    return;
                }
            }
