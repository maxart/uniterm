...
impl Server {
    ...
    /// Resolve a Tab ordinal without changing any selection or scroll position.
    pub(super) fn tab_target(&self, project: ProjectId, tab: u32) -> Option<usize> {
        self.windows
            .iter()
            .enumerate()
            .filter(|(_, window)| window.project == project)
            .nth(tab.checked_sub(1)? as usize)
            .map(|(index, _)| index)
    }

    /// Create in the target Project directly; background work never activates it.
    pub(super) fn create_tab(
        ...
        let root = PathBuf::from(&self.projects.iter().find(|item| item.id == project)?.root);
        let pane = self.spawn_pane_at(reg, &[], Some(&root)).ok()?;
        self.insert_tab(pane, project, background);
        self.finish_targeted_creation(reg, background, false);
        Some(pane)
    }

    /// Attach a new shell to its owner without changing unrelated remembered selections.
    pub(super) fn insert_tab(&mut self, pane: PaneId, project: ProjectId, background: bool) {
        self.windows.push(Win {
            project,
    ...
    /// Targeted splits share the interactive path and inherit their Project root.
    pub(super) fn split_pane(
        ...
        let root = PathBuf::from(&self.projects.iter().find(|item| item.id == project)?.root);
        let new_id = self.spawn_pane_at(reg, &[], Some(&root)).ok()?;
        let dir = match axis {
            SplitAxis::LeftRight => SplitDir::Horizontal,
            SplitAxis::TopBottom => SplitDir::Vertical,
        };
        self.insert_split(window, pane, new_id, dir, background);
        self.finish_targeted_creation(reg, background, window == self.active_window);
        Some(new_id)
    }

    /// Background splits retain the selected Pane and existing zoom.
    pub(super) fn insert_split(
        &mut self,
        window: usize,
        anchor: PaneId,
        pane: PaneId,
        ...
        self.windows[window].layout.split(anchor, dir, pane);
        if !background {
            self.last_active_pane = Some(self.windows[window].active);
            self.windows[window].active = pane;
            self.windows[window].zoomed = None;
            self.activate_window(window);
        }
    }

    /// Hidden creation updates chrome only; visible layout changes resize once.
    pub(super) fn finish_targeted_creation(
        &mut self,
        reg: &Registry,
        background: bool,
        visible: bool,
    ...
    /// Targeted and interactive Tab closure use the same process teardown.
    pub(super) fn close_tab(&mut self, reg: &Registry, window: usize) {
        let panes = self.windows[window].layout.pane_ids();
    ...
    pub(super) fn move_active_tab(&mut self, direction: TabMoveDirection) -> bool {
        self.move_tab(self.active_window, direction)
    }

    /// Reordering must keep the same selected Tab even when its neighbor moves.
    pub(super) fn move_tab(&mut self, window: usize, direction: TabMoveDirection) -> bool {
        let project = self.windows[window].project;
        let tabs = self.project_window_indices(project);
        if tabs.len() < 2 {
            return false;
        }
        let Some(from) = tabs.iter().position(|index| *index == window) else {
            return false;
        };
        let Some(to) = tab_move_target(tabs.len(), from, direction) else {
            return false;
        };
        self.move_tab_to(window, to)
    }

    /// Move a Tab to a Project-relative position, retaining every other Tab's
    /// order and the selected Pane. Mouse and directional controls share this path.
    pub(super) fn move_tab_to(&mut self, window: usize, to: usize) -> bool {
        let Some(tab) = self.windows.get(window) else {
            return false;
        };
        let project = tab.project;
        let tabs = self.project_window_indices(project);
        ...
        let steps: Vec<_> = if from < to {
            (from..to).map(|i| (i, i + 1)).collect()
        } else {
            (to..from).rev().map(|i| (i + 1, i)).collect()
        };
        for (a, b) in steps {
            self.windows.swap(tabs[a], tabs[b]);
            if self.active_window == tabs[a] {
                self.active_window = tabs[b];
            } else if self.active_window == tabs[b] {
                self.active_window = tabs[a];
            }
        }
    ...
    pub(super) fn create_project(
        ...
        self.create_project_with_focus(reg, name, root, false)
    }

    /// Background Project creation leaves the human's view and file manager intact.
    pub(super) fn create_project_with_focus(
        &mut self,
        reg: &Registry,
        name: &str,
        root: &str,
