...
impl Server {
    ...
    pub(super) fn persist_workspace_definition(&mut self) {
        ...
        if let Ok(line) = crate::workspace_catalog::encode(&definition) {
            ...
            if self.last_catalog_line.as_deref() == Some(line.as_str()) {
                return;
            }
            self.agents
    ...
    pub(super) fn snapshot(&self) -> crate::persist::Snapshot {
        let windows = self
            .windows
            .iter()
                ...
                panes: w
                    ...
                    .map(|id| {
                        ...
                        crate::persist::PaneSnap {
                            ...
                            content: pane
                                .map(|p| {
                                    p.term.grid().export_lines(crate::persist::CONTENT_LINE_CAP)
                                })
                                .unwrap_or_default(),
                            metadata: pane
                                .map(|pane| {
    ...
    pub(super) fn persist(&mut self) {
        ...
        if let Ok(bytes) = crate::persist::serialize(&snapshot) {
    ...
    pub(super) fn restore_workspace_definition(
        ...
        for (_, mut pane) in std::mem::take(&mut self.panes) {
            let _ = reg.deregister(&mut SourceFd(&pane.pty.raw_fd()));
            let _ = pane.pty.kill();
        }
