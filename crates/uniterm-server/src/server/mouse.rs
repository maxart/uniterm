...
impl Server {
    pub(super) fn handle_status_mouse(
        ...
        if matches!(kind, MouseKind::Click | MouseKind::RightClick) {
            let left_today = self.close_today();
            if kind == MouseKind::Click {
                self.tab_drag = Some((client, self.windows[window].active));
            }
            if window != self.active_window || left_today {
    ...
    pub(super) fn on_mouse(
        ...
        // Release outside the bar also cancels the drag. Only its initiating
        // client may move the Tab, and a closed source cannot move a replacement.
        if let Some((owner, pane)) = self.tab_drag {
            if owner == client && matches!(kind, MouseKind::Drag | MouseKind::Release) {
                if kind == MouseKind::Release {
                    self.tab_drag = None;
                    let target = self
                        .tab_bar_layout(false)
                        .tabs
                        .into_iter()
                        .find(|slot| slot.rect.contains(cx, cy));
                    let source = self.windows.iter().position(|tab| {
                        tab.project == self.active_project && tab.layout.contains_pane(pane)
                    });
                    if let (Some(source), Some(target)) = (source, target) {
                        if self.move_tab_to(source, target.item) {
            ...
            if owner == client && kind == MouseKind::Click {
                self.tab_drag = None;
            }
        }
        ...
        if self.today.is_some() && self.chrome_area().0.contains(cx, cy) {
            self.today_mouse(reg, cx, cy, kind);
            return;
        }
        ...
        if self.context_menu.is_some() {
            self.handle_context_mouse(reg, client, cx, cy, kind);
            return;
        }
            ...
            match self.observatory_tab {
                ObservatoryTab::Agents => {
                    ...
                    if matches!(kind, MouseKind::WheelUp | MouseKind::WheelDown) {
                        ...
                        if let Some(target) = self.agent_context_target(cy) {
                            self.run_agent_context_action(
                                reg,
                                client,
                                target,
                ...
                ObservatoryTab::WebServers => {
                    ...
                    if matches!(kind, MouseKind::WheelUp | MouseKind::WheelDown) {
                        ...
                        if let Some(target) = self.dev_server_context_target(cy) {
                            self.run_dev_server_action(
                                reg,
                                client,
                                target,
