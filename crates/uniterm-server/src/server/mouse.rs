...
impl Server {
    ...
    pub(super) fn on_mouse(
        ...
        match kind {
            ...
            MouseKind::Release if self.finish_selection(reg) => return,
            _ => {}
        }
        ...
        match kind {
            MouseKind::Hover | MouseKind::Click => {
                ...
                if kind == MouseKind::Click {
                    self.arm_selection(client, pid, cx, cy);
                }
    ...
    /// Arm a text selection at a press, unless the pane's app owns the mouse
    /// (it received the press instead; vim with `mouse=a` does its own
    /// selection). A pane frozen in copy-mode is always selectable.
    pub(super) fn arm_selection(&mut self, client: Token, pane_id: PaneId, cx: u16, cy: u16) {
        let Some(pane) = self.panes.get(&pane_id) else {
            return;
        };
        if pane.copy.is_none() && pane.term.mouse_mode() != MouseMode::Off {
            self.mouse_sel = None;
            return;
        }
    ...
    pub(super) fn drag_selection(&mut self, reg: &Registry, cx: u16, cy: u16) -> bool {
        ...
        if !ms.selecting {
            copy.mouse_anchor(rect, ms.press.0, ms.press.1);
            ms.selecting = true;
        }
    ...
    /// Finish a click-drag selection on release: yank it to the clipboard
    /// (OSC 52) and return the pane to the live screen. `false` when no drag
    /// selected anything (a plain click).
    pub(super) fn finish_selection(&mut self, reg: &Registry) -> bool {
        ...
        if let Some(pane) = self.panes.get_mut(&ms.pane) {
            if let Some(copy) = pane.copy.take() {
