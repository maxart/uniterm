...
impl Server {
    ...
    pub(super) fn on_pty(&mut self, reg: &Registry, token: Token, readable: bool, writable: bool) {
        ...
        if let Some(pane) = self.panes.get_mut(&pane_id) {
            ...
            // The file opener emits this boundary before returning to a shell.
            // Clear only the live invocation, without polling editor processes.
            for mark in pane.term.take_prompt_marks() {
                if mark == "D;uniterm-editor" {
                    if let Some(key) = pane.editor_file.take() {
            ...
            for ev in pane.term.take_agent_events() {
                ...
                match (ev.agent, pane.agent.as_mut()) {
                    (Some(id), _) => {
                        ...
                        if pane.agent.as_ref().is_none_or(|agent| agent.id != id) {
            ...
            if got_output {
                ...
                if process_changed
                    && pane.foreground_pid.is_some()
                    && pane.pty.child_owns_foreground()
                    && pane.term.is_alt_screen()
                {
                    // Recover only after observing a foreground job return
                    // to the pane child. The first observation is not a
                    // return: that child may itself be an editor or pager,
                    // whose alternate screen and input modes must survive.
...
/// Text and Enter are separate terminal events: Enter is CR and must be
/// outside bracketed paste delimiters on Linux, macOS, and Termux alike.
pub(super) fn paste_input(text: &str, bracketed: bool, submit: bool) -> Vec<u8> {
    let mut bytes = Vec::with_capacity(text.len().saturating_add(13));
...
#[cfg(test)]
mod paste_tests {
    use super::paste_input;

    #[test]
    fn enter_is_outside_multiline_paste_and_empty_submit_is_only_enter() {
        assert_eq!(
            paste_input("first\nsecond", true, true),
            b"\x1b[200~first\nsecond\x1b[201~\r"
        );
        assert_eq!(paste_input("hello", false, true), b"hello\r");
        assert_eq!(
            paste_input("hello", true, false),
            b"\x1b[200~hello\x1b[201~"
        );
        assert_eq!(paste_input("", true, true), b"\r");
        assert!(paste_input("", false, false).is_empty());
    }
}
