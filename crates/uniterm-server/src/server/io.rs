...
impl Server {
    ...
    pub(super) fn on_pty(&mut self, reg: &Registry, token: Token, readable: bool, writable: bool) {
        ...
        if eof {
            ...
            // An agent appeared or closed, so repaint the event-driven fleet
            // chrome without resizing or disturbing the terminal application.
    ...
    pub(super) fn on_client(
        ...
        if writable {
            ...
            if let Some(client) = clients.get_mut(&token) {
                ...
                remove |= client.dead;
                if !client.dead && !client.wants_write() && client.repaint_pending {
    ...
    pub(super) fn terminate_panes(&mut self, panes: &[PaneId]) {
        ...
        for pane in panes {
            if let Some(pane) = self.panes.get_mut(pane) {
                signaled |= pane.pty.request_terminate().unwrap_or(false);
            }
        }
        if signaled {
            // One collective grace period keeps shutdown latency constant as
            // pane count grows while still allowing orderly TERM handling.
            std::thread::sleep(std::time::Duration::from_millis(15));
        }
        for pane in panes {
            if let Some(pane) = self.panes.get_mut(pane) {
                let _ = pane.pty.finish_terminate();
            }
        }
    }
