    ...
    pub(super) fn on_control_request(
        ...
        let response = if request.version != uniterm_proto::CONTROL_API_VERSION {
            ...
            match request.command {
                ControlCommand::TimelineQuery { date, filter, before } => {
                    if date.len() > 32 || filter.len() > 256 {
                        ControlResponse::error(request.id, "invalid_date", "date must be today or YYYY-MM-DD")
                    } else {
                        self.agents.send(uniterm_proto::CoreToAgent::TimelineQuery {
                            name: self.name.clone(), date, filter, before, through: self.log.current_sequence(),
                            requester: uniterm_proto::TimelineRequester::Control { connection, id: request.id },
                        });
                        return;
                    }
                }
                ControlCommand::TimelineResume { sequence } => {
                    if sequence == 0 || sequence > self.log.current_sequence() {
                        ControlResponse::error(request.id, "invalid_event", "Choose an existing history event")
                    } else {
                        self.agents.send(uniterm_proto::CoreToAgent::TimelineResume { name: self.name.clone(), sequence, requester: uniterm_proto::TimelineRequester::Control { connection, id: request.id } });
                        return;
                    }
                }
                ControlCommand::TimelineManager { agent, project, manage, background } => {
                    let pane = self.launch_today_manager(reg, &agent, project, manage, background);
                    ControlResponse::ok(request.id, ControlResult::Mutation { resource: "manager".into(), id: pane.map(|p| p.0), found: pane.is_some(), accepted: pane.is_some() })
                }
                ControlCommand::TimelinePrompt { pane, text } => {
                    if !self.state_protected || !self.event_writes_enabled || self.durability_error.is_some() {
                        ControlResponse::error(request.id, "protected_storage_required", "Prompt retention requires unlocked protected storage and a healthy event log")
                    } else if text.is_empty() || text.len() > 4096 || text.chars().any(|c| c.is_control() && c != '\n' && c != '\t') {
                        ControlResponse::error(request.id, "invalid_prompt", "Prompt must contain 1..4096 bytes of text")
                    } else if self.panes.get(&pane).is_none_or(|p| p.agent.is_none()) {
                        ControlResponse::error(request.id, "not_found", "Choose a live agent Pane")
                    } else {
                        let context = self.timeline_context(pane);
                        let accepted = self.panes.get_mut(&pane).is_some_and(|target| {
                            let bytes = super::io::paste_input(&text, target.term.bracketed_paste(), true);
                            Self::queue_pane_input(reg, target, &bytes)
                        });
                        if accepted { self.append_event(crate::eventlog::LogEvent::TimelinePrompt { context, text }); }
                        ControlResponse::ok(request.id, ControlResult::Mutation { resource: "timeline_prompt".into(), id: accepted.then(|| self.log.current_sequence()), found: true, accepted })
                    }
                }
                ControlCommand::TimelineOpen => {
                    self.open_today(reg);
                    ControlResponse::ok(request.id, ControlResult::Mutation { resource: "timeline".into(), id: None, found: true, accepted: true })
                }
                ControlCommand::TimelineFocus { pane, invocation } => {
                    let found = self.timeline_invocations.get(&pane) == Some(&invocation);
                    let accepted = found && self.focus_pane_target(reg, pane);
                    ControlResponse::ok(request.id, ControlResult::Mutation { resource: "pane".into(), id: Some(pane.0), found, accepted })
                }
                ControlCommand::TimelineNote { project, pane, text } => {
                    if !self.event_writes_enabled || self.durability_error.is_some() {
                        ControlResponse::error(request.id, "durability_unavailable", "Cannot save a note while the event log is unavailable")
                    } else if text.trim().is_empty() || text.len() > 4096 || text.chars().any(|c| c.is_control() && c != '\n' && c != '\t') {
                        ControlResponse::error(request.id, "invalid_note", "note must contain 1..4096 bytes of text")
                    } else if let Some(owner) = self.projects.iter().find(|p| p.id == project) {
                        if pane.is_some() {
                            ControlResponse::error(request.id, "invalid_note", "Project notes do not accept a mutable Pane target; use the Today inspector for session notes")
                        } else {
                            let context = uniterm_core::timeline::TimelineContext { project: Some(project), project_name: owner.name.clone(), root: owner.root.clone(), ..Default::default() };
                            self.append_event(crate::eventlog::LogEvent::TimelineNote { context, text });
                            ControlResponse::ok(request.id, ControlResult::Mutation { resource: "timeline_note".into(), id: Some(self.log.current_sequence()), found: true, accepted: true })
                        }
                    } else { ControlResponse::error(request.id, "not_found", "Project not found in this Workspace") }
                }
                ControlCommand::Capabilities => ControlResponse::ok(
                    ...
                    ControlResult::Capabilities {
                        ...
                        capabilities: vec![
                            "timeline.query".into(), "timeline.note".into(), "timeline.focus".into(),
                ...
                ControlCommand::ProjectCreate { name, root, background } => {
                    let accepted = self.create_project_with_focus(reg, &name, &root, background).is_ok();
                    ControlResponse::ok(
                        request.id,
                ...
                ControlCommand::TabCreate { project, background } => {
                    let found = self.projects.iter().any(|item| item.id == project);
                    let before = self.project_window_indices(project).len();
                    let accepted = self.create_tab(reg, project, background).is_some();
                    ControlResponse::ok(
                        request.id,
                        ControlResult::Mutation {
                            resource: "tab".into(),
                ...
                ControlCommand::TabRename { project, tab, name } => {
                    let target = self.tab_target(project, tab);
                    let accepted = target.is_some() && !name.trim().is_empty();
                    if let Some(window) = target.filter(|_| accepted) {
                ...
                ControlCommand::TabMove {
                    ...
                    let target = self.tab_target(project, tab);
                    let accepted = target.is_some_and(|window| self.move_tab(window, direction));
                    if accepted {
                        self.repaint_chrome_all(reg);
                    }
                    ControlResponse::ok(
                ...
                ControlCommand::TabPaste { project, tab, text, submit } => {
                    let pane = self.tab_target(project, tab).map(|window| self.windows[window].active);
                    let accepted = pane.and_then(|pane| self.panes.get_mut(&pane)).is_some_and(|target| {
                        let bytes = super::io::paste_input(&text, target.term.bracketed_paste(), submit);
                        Self::queue_pane_input(reg, target, &bytes)
                    });
                    ControlResponse::ok(request.id, ControlResult::Mutation {
                        resource: "pane".into(), id: pane.map(|pane| pane.0),
                        found: pane.is_some(), accepted,
                    })
                }
                ControlCommand::TabClose { project, tab } => {
                    let target = self.tab_target(project, tab);
                    if let Some(window) = target {
                        self.close_tab(reg, window);
                    }
                    ControlResponse::ok(request.id, ControlResult::Mutation {
                        resource: "tab".into(), id: Some(u64::from(tab)),
                        found: target.is_some(), accepted: target.is_some(),
                    })
                }
                ControlCommand::PaneSplit { pane, axis, background } => {
                    let found = self.panes.contains_key(&pane);
                    let created = self.split_pane(reg, pane, axis, background);
                    ControlResponse::ok(request.id, ControlResult::Mutation {
                        resource: "pane".into(), id: created.map(|pane| pane.0),
                        found, accepted: created.is_some(),
                    })
                }
                ControlCommand::PaneClose { pane } => {
                    let found = self.panes.contains_key(&pane);
                    if found {
                        self.close_pane(reg, pane);
                    }
                    ControlResponse::ok(request.id, ControlResult::Mutation {
                        resource: "pane".into(), id: Some(pane.0), found, accepted: found,
                    })
                }
                ...
                ControlCommand::AgentLaunch { agent, target, pane, background } => {
                    let anchor = pane.unwrap_or(self.windows[self.active_window].active);
                    let pane = self.launch_agent_at(reg, &agent, target, anchor, background);
                    ControlResponse::ok(
                        request.id,
                        ControlResult::Mutation {
                            resource: "agent".into(),
                ...
                ControlCommand::AgentStop { pane } => {
                    let found = self.stop_agent_pane(reg, pane);
                    ControlResponse::ok(
                        request.id,
                        ControlResult::Mutation {
                            resource: "agent".into(),
                ...
                ControlCommand::PanePaste { pane, text, submit } => {
                    let found = self.panes.contains_key(&pane);
                    let accepted = self.panes.get_mut(&pane).is_some_and(|target| {
                        let bytes = super::io::paste_input(&text, target.term.bracketed_paste(), submit);
                        Self::queue_pane_input(reg, target, &bytes)
                    });
                    ControlResponse::ok(request.id, ControlResult::PaneSent { pane, found, accepted })
                }
