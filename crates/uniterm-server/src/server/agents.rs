...
impl Server {
    ...
    pub(super) fn on_process_exit(&mut self, reg: &Registry, token: Token) {
        ...
        // The agent left the fleet but its pane and the layout are unchanged,
        // so refresh the rails in place rather than clearing the screen.
    ...
    pub(super) fn launch_agent(
        ...
        self.launch_agent_at(reg, agent, target, pane, false)
    }

    /// Resolve the launch relative to its owner without temporarily selecting it.
    pub(super) fn launch_agent_at(
        &mut self,
        reg: &Registry,
        agent: &str,
        target: uniterm_proto::LaunchTarget,
        ...
        let window = self
            .windows
            .iter()
            .position(|tab| tab.layout.contains_pane(anchor))?;
        ...
        match target {
            LaunchTarget::CurrentPane => {
                ...
                if p.agent.is_some() || !p.pty.child_owns_foreground() {
                    return self.launch_agent_at(
                        reg,
                        agent,
                        LaunchTarget::NewPane,
                        anchor,
                        background,
                    );
                }
            ...
            LaunchTarget::NewPane | LaunchTarget::NewWindow => {
                ...
                let arg_refs: Vec<&str> = spawn_args.iter().map(String::as_str).collect();
                let root =
                    PathBuf::from(&self.projects.iter().find(|item| item.id == project)?.root);
                let Ok(new_id) = self.spawn_pane_at(reg, &arg_refs, Some(&root)) else {
                    return None;
                };
                if target == LaunchTarget::NewPane {
                    ...
                    self.insert_split(window, anchor, new_id, SplitDir::Horizontal, background);
                } else {
                    self.insert_tab(new_id, project, background);
                }
                self.bind_agent(new_id, &id);
                self.finish_targeted_creation(
                    reg,
                    background,
                    target == LaunchTarget::NewPane && window == self.active_window,
                );
                Some(new_id)
            }
        }
    ...
    pub(super) fn deliver_agent_notification(
        ...
        match self.config.notifications {
            ...
            uniterm_core::NotificationDelivery::Uniterm => {
                ...
                // The toast is chrome drawn over the rails; show it in place
                // rather than clearing the screen (`draw_notification`).
                self.repaint_chrome_all(reg);
            }
    ...
    pub(super) fn on_agent_reply(&mut self, reg: &Registry, reply: uniterm_proto::AgentToCore) {
        match reply {
            uniterm_proto::AgentToCore::TimelineResumeReady { requester, result } => {
                self.finish_timeline_resume(reg, requester, result)
            }
            uniterm_proto::AgentToCore::TimelineLoaded { requester, result } => {
                self.timeline_loaded(reg, requester, result)
            }
            uniterm_proto::AgentToCore::TimelineChanged { sequence } => {
                if let Some(view) = self.today.as_mut() {
                    view.dirty_sequence = sequence;
                    if !view.pending {
            ...
            uniterm_proto::AgentToCore::EditorResolved {
                ...
                let key = (project, canonical_path);
                if reuse_tab {
                    if let Some(pane) = self.editor_panes.get(&key).copied() {
                        if self.panes.contains_key(&pane) && self.focus_pane_target(reg, pane) {
                            return;
                        }
                        self.editor_panes.remove(&key);
                    }
                }
                if let Some(error) = error {
                ...
                if !reuse_tab {
                    self.agents
                        .send(uniterm_proto::CoreToAgent::EditorLaunchExternal {
                            project,
                            path,
                            command,
                            cwd: self.files.root.clone(),
                        });
                    return;
                }
                let completion = "printf '\\033]133;D;uniterm-editor\\007'; ";
                let launch = format!(
                    "{command} -- {}; {completion}exec {}",
                ...
                match self.spawn_pane(reg, &args) {
                    Ok(pane) => {
                        self.panes.get_mut(&pane).unwrap().editor_file = Some(key.clone());
                        self.editor_panes.insert(key, pane);
            ...
            uniterm_proto::AgentToCore::EditorExternalFinished { project, error } => {
                if project == self.files.project && project == self.active_project {
                    if let Some(error) = error {
                        self.files.error = Some(error);
                        if self.file_manager_visible() {
            ...
            uniterm_proto::AgentToCore::DevServerStopFinished {
                pane,
                port,
                detected,
                error,
                ...
                if let Some(body) = error {
                    self.notification = Some(AgentToast {
                        pane,
                        title: format!("Could not stop server :{port}"),
                        body,
                        expires: std::time::Instant::now() + std::time::Duration::from_secs(8),
                    });
                    self.repaint_chrome_all(reg);
                    ...
                    self.agents
                        .send(uniterm_proto::CoreToAgent::DevServerForget { pane, port });
                    self.dev_servers.remove(&(pane, port));
                    self.broadcast_dev_servers(reg);
                }
            }
            ...
            uniterm_proto::AgentToCore::WorkspaceCatalogSaved { name, line, error } => {
                if name == self.name {
                    if self.pending_catalog_line.as_deref() == Some(line.as_str()) {
                    ...
                    if let Some(error) = error {
                        self.on_agent_reply(
                            ...
                            uniterm_proto::AgentToCore::DurabilityError {
                                workspace: name,
                                operation: "Workspace catalog save".into(),
                                error,
                            },
                        );
                    } else {
                        self.last_catalog_line = Some(line);
                    }
                }
            }
        ...
        if let Some(id) = detected_agent {
            ...
            if replace {
                ...
                if let Some((name, line)) = self.log.record(crate::eventlog::LogEvent::AgentBound {
                    ...
                    self.timeline_invocations
                        .insert(pane_id, self.log.current_sequence());
                    self.agents
    ...
    pub(super) fn close_pane(&mut self, reg: &Registry, pane_id: PaneId) {
        // Project removal selects its replacement before closing old Panes;
        // their old layout is still on screen until this first relayout.
        let visible = self.windows[self.active_window]
            .layout
        ...
        if let Some(pane) = self.panes.remove(&pane_id) {
            if let Some(key) = pane.editor_file {
                self.editor_panes.remove(&key);
            }
