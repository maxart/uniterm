...
pub use socket::{
    config_path, default_socket_path, load_config, run_server, run_server_with_ready, socket_dir,
    WorkspaceLock,
};
...
/// Longest a stopping server waits for clients to take their final frames.
const CLIENT_DRAIN_BOUND: std::time::Duration = std::time::Duration::from_secs(2);
...
struct Pane {
    ...
    /// Live file-manager editor invocation; never restored into a plain shell.
    editor_file: Option<(ProjectId, String)>,
}
...
struct Client {
    ...
    /// Whether the Exited notice has been queued for this client. It goes out
    /// last, after any collapsed repaint, so a client never exits on it with a
    /// frame still owed.
    exit_notified: bool,
    /// The last frame that still showed a pane which has since closed, kept
    /// for a client whose queue could not take it at the time. It is sent
    /// ahead of any regenerated repaint, which can no longer contain that
    /// pane's output.
...
pub struct Server {
    ...
    /// File identity to live editor invocation, scoped to this Workspace/Project.
    editor_panes: HashMap<(ProjectId, String), PaneId>,
    ...
    /// Last successfully written Workspace definition, acknowledged by the
    /// runtime. Enqueueing a save alone must never suppress retries.
    last_catalog_line: Option<String>,
    /// Latest unacknowledged definition. A return to an older saved layout
    /// must supersede a different definition that is still in flight.
    pending_catalog_line: Option<String>,
    ...
    retiring_ptys: HashMap<Token, reaper::RetiringPty>,
    retiring_exits: HashMap<Token, Token>,
    sock_path: PathBuf,
    ...
    /// A Tab drag is pinned to a Pane, not an ordinal that automation can move.
    tab_drag: Option<(Token, PaneId)>,
    ...
    today: Option<today::Today>,
    today_generation: u64,
    timeline_invocations: HashMap<PaneId, u64>,
...
struct MouseSel {
    ...
    press: (u16, u16),
    selecting: bool,
    /// The press was withheld from an app that owns the mouse: a release
    /// without a drag delivers it (with the release) as a plain click.
    deferred_press: bool,
}
...
impl Server {
    ...
    fn bind_internal(
        ...
        let mut server = Server {
            ...
            next_project_id: 2,
            project_scroll: 0,
            sidebar_agent_scope: SidebarScope::Workspace,
            sidebar_server_scope: SidebarScope::Workspace,
            ...
            pending_catalog_line: None,
            pending_reads: Vec::new(),
            retiring_ptys: HashMap::new(),
            retiring_exits: HashMap::new(),
            ...
            today: None,
            today_generation: 0,
            timeline_invocations: HashMap::new(),
            state_protected: crate::privacy::is_protected(name),
            workflows: Vec::new(),
            relays: Vec::new(),
    ...
    pub fn run(&mut self, poll: &mut Poll) -> std::io::Result<()> {
        self.append_event(crate::eventlog::LogEvent::TimelineBoundary {
            reason: "Workspace started; earlier open invocations may have been interrupted".into(),
        });
        self.persist();
        // Restored providers are new processes even when their session ID is
        // unchanged. Recovery ran with event writes disabled; establish fresh
        // invocation identities after the startup boundary and context record.
        let restored: Vec<_> = self
            .panes
            .iter()
            .filter_map(|(pane, p)| {
                p.agent.as_ref().map(|a| {
                    (
                        *pane,
                        a.id.clone(),
                        a.session_id.clone(),
                        a.resume_command.clone(),
                    )
                })
            })
            .collect();
        for (pane, provider, session_id, resume_command) in restored {
            self.append_event(crate::eventlog::LogEvent::AgentBound {
                pane: pane.0,
                agent: provider.clone(),
            });
            self.append_event(crate::eventlog::LogEvent::AgentSessionObserved {
                pane: pane.0,
                provider,
                session_id,
                resume_command,
        ...
        let result = self.run_loop(poll);
        self.drain_clients_before_exit(poll);
        // Errors preserve the crash checkpoint but must still reap children.
        for (_, pane) in std::mem::take(&mut self.panes) {
            let _ = poll
                .registry()
                .deregister(&mut SourceFd(&pane.pty.raw_fd()));
            self.retire_pty(poll.registry(), pane.pty);
        }
        self.drain_retiring_ptys(poll);
        result
    }

    fn run_loop(&mut self, poll: &mut Poll) -> std::io::Result<()> {
        ...
        while self.running {
            ...
            let timeout = if accepts_pending
                || (!self.pending_reads.is_empty() && !self.agents.backpressured())
            {
            ...
            for ev in events.iter() {
                ...
                let token = ev.token();
                if self.on_retiring_pty(reg, token) {
                    continue;
    ...
    /// Deliver what each client is still owed once the loop has stopped: a
    /// repaint collapsed while its socket was backpressured, then the Exited
    /// notice, then the bytes themselves. Pending output used to die with the
    /// socket, so a client that was momentarily behind lost the last frame of
    /// a pane whose exit stopped the Workspace; on macOS, where a Unix socket
    /// buffers only a few KiB, that was the common case. The wait is bounded
    /// so a client that never reads cannot hold the stop open.
    fn drain_clients_before_exit(&mut self, poll: &mut Poll) {
        let deadline = std::time::Instant::now() + CLIENT_DRAIN_BOUND;
        let mut events = Events::with_capacity(64);
        loop {
            let mut pending = false;
            let tokens: Vec<Token> = self.clients.keys().copied().collect();
            for token in tokens {
                let repaint = match self.clients.get_mut(&token) {
                    Some(client) if !client.dead => {
                        client.flush_ready();
                        if client.dead || client.wants_write() {
                            false
                        } else if let Some(frame) = client.final_frame.take() {
                            // The last picture of a pane that has closed is
                            // the last picture, full stop: nothing regenerated
                            // after it could show more.
                            ...
                            if !client.exit_notified {
                                client.exit_notified = true;
                ...
                if repaint {
                    self.full_repaint_client(poll.registry(), token);
                }
                if let Some(client) = self.clients.get_mut(&token) {
                    if !client.dead {
                        pending |= client.wants_write() || !client.exit_notified;
                        let _ = set_interest(poll.registry(), client, token);
                    }
                }
            }
            if !pending {
    ...
    fn flush_detection_due(&mut self, reg: &Registry) {
        ...
        if changed {
            // A smoothed status detection changed a fleet pill; the layout is
            // unchanged, so refresh the rails in place instead of clearing the
            // screen. This is the dominant per-status-change path for a
            // screen-detected agent, so a full repaint here flickered the
            // sidebars on every working/idle flip.
        ...
        if metadata_changed {
            // Expired agent metadata badges are chrome; update in place.
            self.repaint_chrome_all(reg);
            self.persist();
        }
    ...
    fn settings_snapshot(&self) -> uniterm_proto::SettingsSnapshot {
        uniterm_proto::SettingsSnapshot {
            ...
            notification_sound: self.config.notification_sound.name().to_string(),
            notification_sounds: uniterm_core::NotificationSound::ALL
                .iter()
                .map(|sound| sound.name().to_string())
                .collect(),
            notification_sound_file: self.config.notification_sound_file.clone(),
            focus_follows_mouse: self.config.focus_follows_mouse,
            freeze_on_select: self.config.freeze_on_select,
            copy_on_select: self.config.copy_on_select,
    ...
    fn open_context_menu(&mut self, reg: &Registry, cx: u16, cy: u16) {
        ...
        let target = if self.observatory_tab == ObservatoryTab::Agents
            && file_sidebar > 0
            && cx >= self.cols.saturating_sub(file_sidebar)
        {
            self.agent_context_target(cy)
        } else if self.observatory_tab == ObservatoryTab::WebServers
            && file_sidebar > 0
            && cx >= self.cols.saturating_sub(file_sidebar)
        {
            self.dev_server_context_target(cy)
        } else if self.file_manager_visible()
            && file_sidebar > 0
            && cx >= self.cols.saturating_sub(file_sidebar)
        {
            self.files.focused = true;
        ...
        match kind {
            ...
            MouseKind::Click | MouseKind::Release => {
                ...
                if let Some(action) = menu.action_at(self.cols, self.rows, cx, cy) {
                    self.run_context_action(reg, client, menu, action);
                } else {
                    self.full_repaint_all(reg);
                }
            }
    ...
    fn handle_context_input(&mut self, reg: &Registry, client: Token, input: &[u8]) {
        let result = self
            .context_menu
            .as_mut()
    ...
    fn run_context_action(
        ...
        match action {
            ContextAction::FocusAgent | ContextAction::CopyAgentPath | ContextAction::StopAgent => {
                self.run_agent_context_action(reg, client, menu.target, action);
                self.full_repaint_all(reg);
            }
            ContextAction::OpenServer | ContextAction::FocusServer | ContextAction::StopServer => {
                self.full_repaint_all(reg);
                self.run_dev_server_action(reg, client, menu.target, action);
            }
    ...
    fn reply_settings(&mut self, reg: &Registry, token: Token, saved: bool, error: Option<String>) {
        let message = ServerMessage::Settings {
            settings: Box::new(self.settings_snapshot()),
            saved,
            error,
        };
        if let Some(client) = self.clients.get_mut(&token) {
    ...
    fn apply_settings(
        ...
        if let Some(sound) = patch.notification_sound {
            self.config.notification_sound = uniterm_core::NotificationSound::parse(&sound);
        }
        if let Some(file) = patch.notification_sound_file {
            self.config.notification_sound_file = file.trim().to_string();
        }
        if let Some(focus) = patch.focus_follows_mouse {
            self.config.focus_follows_mouse = focus;
        }
        if let Some(freeze) = patch.freeze_on_select {
            self.config.freeze_on_select = freeze;
        }
        if let Some(copy) = patch.copy_on_select {
            self.config.copy_on_select = copy;
        }
    ...
    fn enter_copy_mode(&mut self, reg: &Registry) {
        ...
        if let Some(pane) = self.panes.get_mut(&active) {
            if pane.copy.is_none() {
                let mut copy = CopyState::new(pane.term.grid(), rect);
    ...
    fn shutdown(&mut self, _reg: &Registry) {
        // Clients learn of the stop from `drain_clients_before_exit`, after
        // the loop ends: a collapsed repaint they are still owed goes first,
        // then Exited, and both are actually written.
    ...
    fn rename_session(&mut self, reg: &Registry, name: &str) {
        ...
        self.name = new;
        self.last_catalog_line = None;
        self.pending_catalog_line = None;
...
mod tests {
    ...
    fn osc7_working_directories_are_absolute_and_percent_decoded() {
        assert_eq!(
            osc7_working_directory("file://work-laptop/Users/dev/Work/Uniterm%20CLI"),
            Some(PathBuf::from("/Users/dev/Work/Uniterm CLI"))
        );
        assert_eq!(
