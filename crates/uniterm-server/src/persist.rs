...
pub use uniterm_proto::checkpoint::{AgentLaunchSnap, PaneSnap, ProjectSnap, Snapshot, WinSnap};
...
const VERSION: u32 = uniterm_proto::checkpoint::SNAPSHOT_VERSION;
...
// Library tests share one process. Resolve their state once without consulting
// mutable environment variables, including before any test has set up a server.
// Integration tests exercise the production XDG lookup with `isolate_state()`.
#[cfg(test)]
pub(crate) fn state_dir() -> PathBuf {
    ...
    STATE
        .get_or_init(|| {
            let nonce = std::time::SystemTime::now()
                ...
                .unwrap_or_default()
                .as_nanos();
            std::env::temp_dir().join(format!("uniterm-lib-state-{}-{nonce}", std::process::id()))
        })
        .clone()
}
...
/// Persist a newly published filename before acknowledging a durable save.
...
/// Serialize an owned projection on the persistence worker. Compact captures
/// stream the existing cell schema without allocating a String for each cell.
...
/// Publish a compact capture using the same schema without expanding a fleet
/// of Strings. Called only on the ordered persistence worker.
pub(crate) fn save_capture(
    name: &str,
    snapshot: &Snapshot<uniterm_core::GridCapture>,
) -> std::io::Result<()> {
    let bytes =
        bincode::serialize(snapshot).map_err(|error| std::io::Error::other(error.to_string()))?;
...
pub fn save_bytes(name: &str, bytes: &[u8]) -> std::io::Result<()> {
    ...
    write_private(&tmp, &crate::privacy::encode(name, bytes)?)?;
...
pub fn load(name: &str) -> Option<Snapshot> {
    ...
    decode_snapshot(&crate::privacy::decode(&bytes).ok()?)
}
