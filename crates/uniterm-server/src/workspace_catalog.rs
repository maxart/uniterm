...
pub(crate) fn privacy_path(name: &str) -> PathBuf {
    path(name)
}
...
pub fn append_line(name: &str, line: &str) -> std::io::Result<()> {
    ...
    file.write_all(crate::privacy::encode_line(name, line)?.as_bytes())?;
...
fn latest_valid_line(contents: &str) -> Option<(&str, WorkspaceDefinition)> {
    contents.lines().rev().find_map(|line| {
        let decoded = crate::privacy::decode_line(line).ok()?;
        serde_json::from_str::<WorkspaceDefinition>(&decoded)
            .ok()
