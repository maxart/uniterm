...
impl PtyProcess {
    ...
    /// Escalate without waiting. The mio reaper keeps reading the master and
    /// uses native child-exit readiness to finish ownership asynchronously.
    pub(crate) fn force_terminate(&mut self) -> std::io::Result<()> {
        if self.termination_finished {
            return Ok(());
        }
        let group_signaled = self.signal_group(libc::SIGKILL)?;
    ...
    /// Start asynchronous teardown without reaping the leader. Retaining its
    /// pid until the final group signal prevents pid reuse during the grace.
    pub(crate) fn begin_terminate(&mut self) -> std::io::Result<()> {
        if !self.signal_group(libc::SIGTERM)? {
            let _ = self.signal_process(libc::SIGTERM)?;
        }
        Ok(())
    }

    /// The owned shell pid, used only to register a native exit watch.
