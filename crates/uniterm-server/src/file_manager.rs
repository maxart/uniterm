...
impl FileManager {
    ...
    /// Open the clicked item immediately, using the same action as Enter.
...
mod tests {
    ...
    #[test]
    fn first_click_opens_unselected_rows_and_ignores_empty_space() {
        ...
        assert!(matches!(manager.click(1, 0, 4), FileAction::Open(path) if path == "/work/b.txt"));
        assert_eq!(manager.selected, 1);
        assert!(matches!(manager.click(0, 1, 4), FileAction::Open(path) if path == "/work/b.txt"));
        assert!(matches!(manager.click(3, 0, 4), FileAction::None));
        assert!(matches!(manager.click(4, 0, 4), FileAction::None));
        assert_eq!(manager.selected, 1);
    }
