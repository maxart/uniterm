...
//!
//! The viewport addresses logical lines of history plus screen, which holds
//! still while output scrolls. With the `freeze-on-select` setting the screen
//! rows are also copied the moment a selection starts, so an application that
//! repaints in place, or owns the alternate screen, cannot change the text
...
/// The live screen rows as they were when a selection began, held for the
/// `freeze-on-select` setting. `start` is the logical index of the first
/// frozen row and follows history eviction; `rows` is a standalone copy of
/// the rows. While a freeze is held the viewport ends at these rows and reads
/// them from the copy, so live output can neither scroll nor repaint the text
/// being selected. History above the rows stays live: it only ever shrinks
/// from the top, which `sync_history` compensates.
struct FrozenScreen {
    start: usize,
    rows: Grid,
}
...
pub struct CopyState {
    ...
    /// Snapshot the screen rows when a selection starts (`freeze-on-select`).
    freeze_on_select: bool,
    frozen: Option<FrozenScreen>,
    /// A mouse selection was released with `copy-on-select` off: it stays
    /// highlighted until a key copies or dismisses it, or a plain click does.
    mouse_finished: bool,
}
...
impl CopyState {
    ...
    pub fn new(grid: &Grid, rect: Rect) -> Self {
        ...
        CopyState {
            ...
            frozen: None,
            mouse_finished: false,
        }
    }

    /// Record that the mouse released a selection which is being kept on
    /// screen rather than yanked (`copy-on-select` off).
    pub fn finish_mouse_selection(&mut self) {
        self.mouse_finished = true;
    }

    /// Whether a kept mouse selection is showing, so a plain click dismisses
    /// it instead of starting another.
    pub fn mouse_selection_finished(&self) -> bool {
        self.mouse_finished
    }

    /// Apply the `freeze-on-select` setting: whether starting a selection (a
    /// mouse drag, or `v` at the keyboard) copies the live screen rows and
    /// reads them from the copy until the selection ends. Off, copy-mode
    /// addresses logical lines, which holds still under scrolling output but
    /// shows an application's in-place repaints and, on the alternate screen,
    /// every change it makes.
    pub fn set_freeze_on_select(&mut self, on: bool) {
        self.freeze_on_select = on;
    }

    /// Whether a frozen copy of the screen rows is currently held.
    pub fn is_frozen(&self) -> bool {
        self.frozen.is_some()
    }

    fn freeze_if_configured(&mut self, grid: &Grid) {
        if self.freeze_on_select && self.frozen.is_none() {
            self.frozen = Some(FrozenScreen::capture(grid));
        }
    }

    /// Release the frozen rows: the viewport addresses the live buffer again
    /// at the same logical lines.
    fn thaw(&mut self) {
        self.frozen = None;
    }

    /// Logical lines the viewport can address: the live buffer, or with a
    /// freeze the history above the frozen rows plus the rows themselves.
    fn total_lines(&self, grid: &Grid) -> usize {
        match &self.frozen {
            Some(frozen) => frozen.start + frozen.rows.height() as usize,
            None => grid.total_lines(),
        }
    }

    /// The grid holding logical `line` and the line's index there: history
    /// comes from the live grid, the screen from the freeze while one is held.
    fn source<'a>(&'a self, grid: &'a Grid, line: usize) -> (&'a Grid, usize) {
        match &self.frozen {
            Some(frozen) if line >= frozen.start => (&frozen.rows, line - frozen.start),
            _ => (grid, line),
        }
    }
    ...
    /// Shift viewport coordinates down by `evicted` history lines. Frozen rows
    /// are never evicted (the freeze owns a copy), so a line inside the freeze
    /// keeps its offset from the freeze's start, which itself stops at zero.
    fn evict(&mut self, evicted: usize) {
        let frozen_start = self.frozen.as_ref().map(|frozen| frozen.start);
        let shift = |line: usize| match frozen_start {
            Some(start) if line >= start => Some(start.saturating_sub(evicted) + (line - start)),
            _ => line.checked_sub(evicted),
        };
        self.top = shift(self.top).unwrap_or(0);
        self.anchor = self
            .anchor
            .and_then(|(line, x)| shift(line).map(|line| (line, x)));
        if let Some(frozen) = &mut self.frozen {
            frozen.start = frozen.start.saturating_sub(evicted);
        }
    }

    /// Whether the viewport shows the live screen, judged against the grid
    /// copy-mode was last synchronized with. Ask this before a reflow so
    /// `resize` can keep a viewer who was following live output at the bottom
    /// once the row count has changed under it.
    pub fn at_bottom(&self, grid: &Grid) -> bool {
    ...
    /// `at_bottom` is the pre-reflow answer of [`CopyState::at_bottom`]; the
    /// reflowed grid carries fresh line ids, so the eviction arithmetic of
    /// `sync_history` must not run across it.
    pub fn resize(&mut self, grid: &Grid, rect: Rect, at_bottom: bool) {
        self.width = rect.w.max(1);
        self.height = rect.h.max(1);
        // Reflow rewrote the rows under a freeze; hold the reflowed screen.
    ...
    fn max_top(&self, grid: &Grid) -> usize {
        self.total_lines(grid).saturating_sub(self.height as usize)
    }
            ...
            match b {
                ...
                b'v' | b' ' => {
                    if self.anchor.take().is_some() {
                        ...
                        self.freeze_if_configured(grid);
                        self.anchor = Some((self.cursor_line(), self.cx));
                    }
                }
                b'y' | 0x0d | 0x0a => {
    ...
    fn down(&mut self, grid: &Grid) {
        let last = self.total_lines(grid).saturating_sub(1);
    ...
    fn is_space(&self, grid: &Grid, x: u16) -> bool {
        let (source, line) = self.source(grid, self.cursor_line());
        source.line_cell(line, x).ch.is_whitespace()
    }
    ...
    /// `rect`: the anchor and the cursor both move there. With
    /// `freeze-on-select` this is where the screen rows are copied, so the
    /// drag selects the text the press landed on.
    pub fn mouse_anchor(&mut self, grid: &Grid, rect: Rect, sx: u16, sy: u16) {
        self.freeze_if_configured(grid);
        self.mouse_finished = false;
    ...
    fn selected_text(&self, grid: &Grid) -> String {
        let Some((s, e)) = self.selection() else {
            ...
            let (source, line) = self.source(grid, self.cursor_line());
            return line_text(source, line, self.width);
        };
        ...
        for line in s.0..=e.0 {
            ...
            let (source, index) = self.source(grid, line);
            let selected = line_text_range(source, index, from, to);
            if to == self.width {
                out.push_str(selected.trim_end());
            } else {
                out.push_str(&selected);
            }
            if line != e.0 && !source.line_wrapped(index) {
                out.push('\n');
            }
        }
        out
    ...
    fn run_search(&mut self, grid: &Grid, forward: bool) {
        ...
        let total = self.total_lines(grid);
        ...
        for step in 1..=total {
            ...
            let (source, index) = self.source(grid, line);
            let text = line_text(source, index, self.width);
            if let Some(byte) = text.find(&self.query) {
                let col = search_column(source, index, self.width, byte);
    ...
    pub fn render(&self, grid: &Grid, rect: Rect, ops: &mut Vec<u8>) {
        for row in 0..rect.h {
            ...
            let mut x = 0;
            while x < rect.w {
                let cell = source.line_cell(index, x);
                ...
                emit_copy_style(
                    cell,
                    source.underline_color(cell.underline_color),
                    selected,
                    ops,
                );
                source.write_cell_display_text(cell, ops);
...
mod tests {
    ...
    #[test]
    fn resize_keeps_a_bottom_viewer_at_the_bottom_across_a_reflow() {
        ...
        assert!(copy.at_bottom(&grid));
        let at_bottom = copy.at_bottom(&grid);
        grid.resize_reflow(5, 3, Cell::default(), (0, 2), false);
        copy.resize(
            &grid,
            Rect {
                x: 0,
        ...
        assert_eq!(copy.top, grid.total_lines() - 3);
        assert_eq!(copy.first_line_id, grid.first_line_id());

        // A viewer scrolled into history stays there, clamped to the new range.
        let mut grid = Grid::new(10, 3);
        for _ in 0..20 {
            grid.set(
                0,
        ...
        assert!(!copy.at_bottom(&grid));
        let at_bottom = copy.at_bottom(&grid);
        grid.resize_reflow(8, 3, Cell::default(), (0, 2), false);
        copy.resize(
            &grid,
            Rect {
                x: 0,
    ...
    fn mouse_selection_anchors_drags_and_yanks() {
        ...
        let mut cs = CopyState::new(&g, rect);
        cs.mouse_anchor(&g, rect, 2, 1); // pane cell (0, 0)
    ...
    fn text_row(grid: &mut Grid, y: u16, text: &str) {
    ...
    /// The printable text of rendered ops with every CSI sequence removed.
    fn visible_text(ops: &[u8]) -> String {
    ...
    fn rendered(cs: &CopyState, grid: &Grid, rect: Rect) -> String {
        let mut ops = Vec::new();
        cs.render(grid, rect, &mut ops);
        visible_text(&ops)
    }

    #[test]
    fn freeze_on_select_holds_the_screen_under_live_output() {
        let mut g = Grid::new(12, 3);
        text_row(&mut g, 0, "HELLOWORLD");
        let rect = Rect::new(0, 0, 12, 3);
        let mut cs = CopyState::new(&g, rect);
        cs.set_freeze_on_select(true);
        assert!(!cs.is_frozen());
        cs.mouse_anchor(&g, rect, 0, 0);
        assert!(cs.is_frozen());
        cs.mouse_drag(rect, 4, 0);

        // The application repaints the pressed row in place, fills the row
        // below, and scrolls twice; none of it reaches the frozen viewport.
        text_row(&mut g, 0, "REPAINTED!");
        text_row(&mut g, 1, "line1");
        g.scroll_up(Cell::default());
        g.scroll_up(Cell::default());
        cs.sync_history(&g);
        assert_eq!(cs.yank(&g), "HELLO");
        let frame = rendered(&cs, &g, rect);
        assert!(frame.contains("HELLOWORLD"), "{frame:?}");
        assert!(!frame.contains("line1") && !frame.contains("REPAINTED"));
        assert_eq!(cs.total_lines(&g), 3);
        assert!(cs.at_bottom(&g));
        assert!(frame.contains("[COPY 1/3]"), "{frame:?}");
        // Wheel-down at the frozen bottom still reports the bottom.
        assert!(cs.scroll(&g, 3));
    }

    #[test]
    fn selection_without_the_setting_shows_live_repaints() {
        let mut g = Grid::new(12, 3);
        text_row(&mut g, 0, "HELLOWORLD");
        let rect = Rect::new(0, 0, 12, 3);
        let mut cs = CopyState::new(&g, rect);
        ...
        g.scroll_up(Cell::default());
        cs.sync_history(&g);
        // Logical lines hold still under the scroll, but the in-place repaint
        // shows through: the default behaviour is unchanged.
        assert_eq!(cs.yank(&g), "REPAI");
        let frame = rendered(&cs, &g, rect);
        assert!(frame.contains("REPAINTED!") && frame.contains("line1"));
        assert_eq!(cs.total_lines(&g), 5);
        assert!(!cs.at_bottom(&g));
    }

    #[test]
    fn frozen_rows_keep_their_offset_when_history_is_evicted() {
        let mut g = Grid::new(8, 2);
        g.set_scrollback_limit(3);
        for i in 0..5 {
            text_row(&mut g, 1, &format!("h{i}"));
            g.scroll_up(Cell::default());
        }
        text_row(&mut g, 0, "S0");
        text_row(&mut g, 1, "S1");
        ...
        cs.set_freeze_on_select(true);
        cs.mouse_anchor(&g, rect, 0, 0);
        cs.mouse_drag(rect, 1, 0);
        assert_eq!(cs.anchor, Some((3, 0)));
        assert_eq!(cs.yank(&g), "S0");

        // Two more scrolls evict h1 and h2 under the freeze.
        g.scroll_up(Cell::default());
        g.scroll_up(Cell::default());
        cs.sync_history(&g);
        assert_eq!(cs.frozen.as_ref().map(|frozen| frozen.start), Some(1));
        assert_eq!(cs.top, 1);
        assert_eq!(cs.anchor, Some((1, 0)));
        assert_eq!(cs.yank(&g), "S0");
        assert_eq!(cs.total_lines(&g), 3);
        // The surviving history line sits above the frozen rows and is live.
        assert!(!cs.scroll(&g, -1));
        let frame = rendered(&cs, &g, rect);
        assert!(frame.contains("h3") && frame.contains("S0"), "{frame:?}");
        assert!(!frame.contains("S1"));
    }

    #[test]
    fn keyboard_selection_freezes_and_cancelling_thaws() {
        let mut g = Grid::new(8, 2);
        text_row(&mut g, 0, "abc");
        let rect = Rect::new(0, 0, 8, 2);
        let mut cs = CopyState::new(&g, rect);
        cs.set_freeze_on_select(true);
        cs.handle(b"k", &g);
        assert!(matches!(cs.handle(b"v", &g), CopyAction::Redraw));
        assert!(cs.is_frozen());
        cs.handle(b"ll", &g);
        text_row(&mut g, 0, "xyz");
        assert_eq!(cs.yank(&g), "abc");
        cs.handle(b"v", &g); // cancel the selection: the live row shows again
        assert!(!cs.is_frozen());
        assert_eq!(cs.yank(&g), "xyz");
        cs.handle(b"hhv", &g);
        assert!(cs.is_frozen());
        text_row(&mut g, 0, "123");
        cs.handle(b"ll", &g);
        match cs.handle(b"y", &g) {
            CopyAction::Copy(text) => assert_eq!(text, "xyz"),
            _ => panic!("expected Copy"),
        }
    }

    #[test]
    fn resize_refreezes_the_reflowed_screen_and_keeps_the_bottom() {
        let mut g = Grid::new(10, 3);
        text_row(&mut g, 0, "abcdefghij");
        let rect = Rect::new(0, 0, 10, 3);
        let mut cs = CopyState::new(&g, rect);
        cs.set_freeze_on_select(true);
        cs.mouse_anchor(&g, rect, 0, 0);
        let at_bottom = cs.at_bottom(&g);
        g.resize_reflow(5, 3, Cell::default(), (0, 2), false);
        cs.resize(&g, Rect::new(0, 0, 5, 3), at_bottom);
        assert!(cs.is_frozen());
        assert!(cs.at_bottom(&g));
        assert_eq!(cs.total_lines(&g), g.total_lines());
        let (source, _) = cs.source(&g, cs.top);
        assert_eq!(source.width(), 5);
        assert_eq!(cs.anchor, None);
    }
