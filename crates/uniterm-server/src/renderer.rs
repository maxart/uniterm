...
impl Renderer {
    ...
    pub fn render(&mut self, grid: &Grid, out: &mut Vec<u8>) -> usize {
        ...
        self.render_pane_damage_precise(grid, 0, 0, out)
    }
    ...
    fn render_pane_damage_precise(
        ...
        for y in 0..grid.height() {
            ...
            self.render_row(grid, y, lo..=hi, (ox, oy), out);
        }
        self.close_hyperlink(out);
        out.len() - start
    }
    ...
    pub fn render_pane_full(&mut self, grid: &Grid, ox: u16, oy: u16, out: &mut Vec<u8>) {
        for y in 0..grid.height() {
            if grid.width() != 0 {
                self.render_row(grid, y, 0..=grid.width() - 1, (ox, oy), out);
            }
        }
        self.close_hyperlink(out);
    }
    ...
    fn render_row(
        ...
        let (lo, hi) = (*columns.start(), *columns.end());
        // ECH is bounded horizontally, unlike EL. It cannot erase a divider or
        // adjacent Pane, and only replaces genuinely default, unlinked blanks.
        let mut text_end = hi + 1;
        while text_end > lo && grid.get(text_end - 1, y) == Cell::default() {
            text_end -= 1;
        }
        if hi + 1 - text_end < 8 {
            text_end = hi + 1;
        ...
        self.cursor = Some((ox + x, oy + y));
        if text_end <= hi {
            self.close_hyperlink(out);
            let blank = Cell::default();
            let style = (blank.fg, blank.bg, blank.attrs, Color::Default);
            if self.last_style != Some(style) {
                self.emit_sgr(&blank, Color::Default, out);
                self.last_style = Some(style);
            }
            // ECH leaves the cursor in place. Do not cache the end of the
            // erased run as if spaces had been printed there.
            let mut sequence = String::new();
            let _ = write!(sequence, "\x1b[{}X", hi + 1 - text_end);
            out.extend_from_slice(sequence.as_bytes());
        }
    }
...
mod tests {
                ...
                if text.as_bytes()[at] == 0x1b && text.as_bytes().get(at + 1) == Some(&b'[') {
                    ...
                    match final_byte {
                        ...
                        b'X' => {
                            ...
                            if let Some(row) = self.cells.get_mut(self.y) {
                                let end = self.x.saturating_add(count).min(row.len());
                                if self.x < end {
                                    row[self.x..end].fill(" ".into());
                                }
                            }
                        }
                        b'r' => {
    ...
    #[test]
    fn blank_tail_erasure_stays_inside_the_pane_and_keeps_cursor_truthful() {
        let mut host = crate::Terminal::new(80, 4);
        host.feed(b"\x1b[1;40HNEIGHBOUR\x1b[2;40HNEIGHBOUR");
        let mut grid = Grid::new(30, 2);
        grid.set(0, 0, plain('A'));
        let mut renderer = Renderer::new();
        let mut output = Vec::new();
        renderer.render_pane_full(&grid, 5, 0, &mut output);
        assert!(output.len() < 80, "blank tails should not be printed");
        assert!(String::from_utf8_lossy(&output).contains("29X"));
        host.feed(&output);
        assert_eq!(host.grid().get(5, 0).ch, 'A');
        for row in 0..2 {
            for (offset, ch) in "NEIGHBOUR".chars().enumerate() {
                assert_eq!(host.grid().get(39 + offset as u16, row).ch, ch);
            }
        }
        grid.clear_damage();
        grid.set(29, 1, plain('Z'));
        output.clear();
        renderer.render_pane_damage(&grid, 5, 0, &mut output);
        host.feed(&output);
        assert_eq!(
            host.grid().get(34, 1).ch,
            'Z',
            "ECH must not advance the cached cursor"
        );
        grid.clear_damage();
        output.clear();
        renderer.render_pane_damage(&grid, 5, 0, &mut output);
        assert!(output.is_empty(), "an unchanged Pane must stay idle");
    }

    #[test]
    fn erasure_preserves_styled_blanks_and_resets_the_background() {
        ...
        let mut host = crate::Terminal::new(40, 2);
        host.feed(&output);
        for x in 0..40 {
            assert_eq!(host.grid().get(x, 0).bg, Color::Idx(4));
        }
        assert_eq!(host.grid().get(0, 1).bg, Color::Idx(1));
        for x in 1..40 {
            assert_eq!(host.grid().get(x, 1), Cell::default());
        }
    }
    ...
    fn full_width_scroll_uses_margins_and_only_repaints_the_exposed_row() {
        ...
        assert!(
            fallback.len() > optimized.len() * 3,
            "scroll must beat repaint"
        );
    }
