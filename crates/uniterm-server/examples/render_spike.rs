...
const LATENCY_BUDGET_US: u128 = 50_000; // 50 ms, the p95 input-to-pixel budget
...
fn main() {
    ...
    let ok = worst < LATENCY_BUDGET_US;
    println!(
        "worst per-frame render: {} us  (budget {} us)  => {}",
        worst,
        LATENCY_BUDGET_US,
...
fn measure_single_pane() -> u128 {
    ...
    let per_frame_us = start.elapsed().as_micros() / FRAMES as u128;
    println!(
        "[5] 1 pane:  {per_frame_us} us/frame, avg {} bytes/frame",
        total_bytes / FRAMES
    );
    per_frame_us
}
...
fn measure_fleet(panes: usize) -> u128 {
    ...
    let per_frame_us = start.elapsed().as_micros() / FRAMES as u128;
    println!("[6] {panes} panes: {per_frame_us} us/frame (renders all panes; clean ones are free)");
    per_frame_us
}
