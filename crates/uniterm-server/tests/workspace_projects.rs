...
fn isolate_state() {
    let state = common::socket_root().join(format!(
...
fn desktop_hierarchy_import_creates_fresh_projects_and_tabs() {
    isolate_state();
    let base = common::socket_root().join(format!(
    ...
    let temp_root = common::socket_root().to_string_lossy().into_owned();
...
fn projects_own_tabs_and_switch_as_one_scope() {
    isolate_state();
    let base =
        common::socket_root().join(format!("uniterm-workspace-projects-{}", std::process::id()));
    std::fs::create_dir_all(&base).unwrap();
...
fn every_new_tab_and_pane_starts_at_the_project_root() {
    isolate_state();
    let base = common::socket_root().join(format!("uniterm-project-cwd-{}", std::process::id()));
