...
fn web_server_scope_toggles_between_project_and_workspace() {
    ...
    stream
        ...
        .unwrap();
    let initial_frame = read_full_frame_until(&mut stream, &mut decoder, |frame| {
        frame.contains("WEB SERVERS")
    });
    assert!(initial_frame.contains("WEB SERVERS 2"));
    assert!(
        initial_frame.contains("workspace"),
        "Servers defaults to Workspace"
    );
    assert!(initial_frame.contains(&first_port_label));
    assert!(initial_frame.contains(&second_port_label));
    stream
        .write_all(&encode_frame(&ClientMessage::Mouse {
            x: 95,
            y: 3,
    ...
    let project_frame = read_full_frame_until(&mut stream, &mut decoder, |frame| {
        frame.contains("WEB SERVERS 1") && frame.contains("project")
    });
