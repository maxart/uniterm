...
fn pane_activity_is_quiet_under_an_overlay_and_reconciles_once_on_close() {
    isolate_state();
    let dir =
        common::socket_root().join(format!("uniterm-overlay-quiescence-{}", std::process::id()));
    ...
    // The fresh shell's prompt and any title follow the attach frame on their
    // own schedule; drain them so a late one cannot be mistaken for activity
    // painted under the overlay.
    while read_render(&mut stream, &mut decoder).is_some() {}
