...
fn chrome_buttons_open_anchored_menus_and_plus_survives_tab_overflow() {
    ...
    assert!(
        frame.contains(&format!("\x1b[0;{};49mworkspace", theme.muted.sgr_fg())),
    ...
    // The fitting Tab fills the strip and the `+` stays at the right edge,
    // including after overflow introduces scroll controls.
    stream
        .write_all(&encode_frame(&ClientMessage::Mouse {
            x: 83,
            y: 1,
