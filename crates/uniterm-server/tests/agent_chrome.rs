...
fn sidebar_agent_scope_toggles_across_projects_and_keeps_project_context() {
    ...
    stream
        ...
        .unwrap();
    let initial_frame = read_frame_until(&mut stream, &mut decoder, "Claude Code");
    assert!(
        initial_frame.contains("workspace"),
        "Agents defaults to Workspace"
    );
    assert!(
        initial_frame.contains("Codex"),
        "default scope includes the other Project"
    );
    stream
