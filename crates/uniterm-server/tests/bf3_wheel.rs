...
fn wheel_scrolls_scrollback_and_returns_to_live() {
    ...
    let server = thread::spawn(move || {
        let (mut s, mut poll) =
            Server::bind(&sock_srv, "/bin/sh", &["-c", "seq 1 60; sleep 30"], 80, 24).unwrap();
        s.set_config(Config {
            sidebar: false,
            ..Config::default()
        });
    ...
    let pre = read_until(&mut stream, &mut dec, 3, |s| s.contains("60"));
    assert!(pre.contains("60"), "pane output never arrived");
    ...
    let far_back = read_until(&mut stream, &mut dec, 3, |s| s.contains("[v Latest]"));
    assert!(
