...
impl Attached {
    fn start() -> (Self, thread::JoinHandle<()>) {
        isolate_state();
        // One directory per test process; the socket name inside it is what
        // must be unique. Embedding a second Workspace name here once pushed
        // the control socket path past the 104-byte macOS limit.
        let dir =
            common::socket_root().join(format!("uniterm-agent-status-{}", std::process::id()));
    ...
    /// Every RenderOps frame that arrives within `window`, as text. Used to
    /// assert what a burst of chrome updates did and did not emit.
    fn drain_frames(&mut self, window: Duration) -> Vec<String> {
        ...
        while Instant::now() < deadline {
            match self.stream.read(&mut buffer) {
                ...
                Err(error) => panic!("drain read failed: {error}"),
            }
            while let Ok(Some(message)) = self.decoder.decode::<ServerMessage>() {
                if let ServerMessage::RenderOps(ops) = message {
...
#[test]
fn agent_binding_and_status_changes_never_clear_the_whole_screen() {
    // The flicker bug: every agent status change repainted the entire frame,
    // clearing the screen and blanking the static rails while a pane streamed.
    // Chrome changes must now update in place with no screen clear.
    let (mut client, server) = Attached::start();

    // The initial attach paint is the one legitimate full-screen clear.
    let attach = client.full_frame_with("");
    assert!(
        attach.contains("\x1b[r\x1b[2J"),
        "the attach frame should be a full repaint"
    );

    // Bind an agent, then drive screen-detected status transitions: a line the
    // Codex rules anchor on (working), then let the shell prompt settle back to
    // idle. The idle settles through the smoothed-detection timer, the dominant
    // per-status-change repaint path. Capture frames long enough to include it.
    client.type_line(&String::from_utf8(announce("codex", "session_start")).unwrap());
    client.type_line("printf 'running command\\n'");
    let frames = client.drain_frames(Duration::from_millis(3500));

    // The bind and any screen-detected status change reach the fleet rail as
    // chrome frames (Codex's card, with its status word).
    assert!(
        frames
            .iter()
            .any(|frame| frame.contains("Codex") && frame.contains("AGENTS")),
        "the bound agent never reached the chrome rails: {frames:?}"
    );
    // The core guarantee: after the one legitimate attach clear, no chrome
    // update - the bind, a working/idle detection, or the smoothed-detection
    // timer that applies a dwelled status - clears the whole screen.
    assert!(
        frames.iter().all(|frame| !frame.contains("\x1b[r\x1b[2J")),
        "a chrome update cleared the whole screen (the flicker): {frames:?}"
    );
