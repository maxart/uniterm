...
pub fn control_request(
    ...
    use std::os::unix::net::UnixStream as StdUnixStream;
    use uniterm_proto::{ControlCommand, ControlRequest, ControlResult};

    fn exchange(
        reader: &mut std::io::BufReader<StdUnixStream>,
        ...
        loop {
            ...
            match serde_json::from_str::<uniterm_proto::ControlFrame>(line.trim_end()) {
    ...
    let workspace = sock_path
        ...
        .to_string();
    let stream = StdUnixStream::connect(sock_path.with_extension("control.sock"))?;
    stream.set_read_timeout(Some(std::time::Duration::from_secs(5)))?;
    let mut reader = std::io::BufReader::new(stream);
    // Older v1 servers ignore unknown JSON fields, including background and
    // explicit Pane anchors. Negotiate on the same connection before sending
    // anything that an older server could reinterpret as a foreground action.
    if matches!(
        &command,
        ControlCommand::ProjectCreate {
            background: true,
            ..
        } | ControlCommand::TabCreate {
            background: true,
            ..
            ...
            background: true,
            ..
        } | ControlCommand::AgentLaunch { pane: Some(_), .. }
            | ControlCommand::TabRename { .. }
        ...
        if !matches!(capabilities.result, Some(ControlResult::Capabilities { capabilities, .. })
            if capabilities.iter().any(|value| value == "automation.background"))
        {
            return Err(std::io::Error::new(std::io::ErrorKind::Unsupported,
                "the running Workspace server does not support background automation; upgrade and restart it before retrying"));
        }
    }
    exchange(
        &mut reader,
