...
pub(crate) fn apply_new_project_action(
    ...
    match action {
        ...
        NewProjectAction::Submit { name, root } => {
            ...
            // the requested path. A remote path must never fail silently. On
            // success the server has already switched to the new Project's
            // Tab, so the client closes its overlays and the user is in it.
...
pub(crate) enum Surface {
    ...
    Projects,
    NewProject,
    /// Boxed: a `ClientMessage` carries a whole settings patch.
    Confirm(Box<ClientMessage>),
    Detach,
}
...
pub(crate) fn run_menu_action(
    ...
    match a {
        ...
        MenuAction::ClosePane if confirm_close => {
            return Surface::Confirm(Box::new(ClientMessage::Command(Command::KillPane)))
        }
        ...
        MenuAction::CloseProject => {
            if let Some(project) = project {
                return Surface::Confirm(Box::new(ClientMessage::ProjectRemove {
                    project,
                    confirmed: true,
                }));
            }
        ...
        MenuAction::CloseTab if confirm_tab_close => {
            return Surface::Confirm(Box::new(ClientMessage::Command(Command::KillWindow)))
        }
        ...
        MenuAction::Today => cmd(Command::Today),
        MenuAction::MoveTabLeft => cmd(Command::MoveTab(uniterm_proto::TabMoveDirection::Previous)),
        MenuAction::MoveTabRight => cmd(Command::MoveTab(uniterm_proto::TabMoveDirection::Next)),
    ...
    fn tab_menu_close_uses_the_tab_confirmation_setting() {
        ...
        assert!(matches!(
            ...
            Surface::Confirm(message)
                if matches!(*message, ClientMessage::Command(Command::KillWindow))
        ));
        assert!(out.is_empty());
