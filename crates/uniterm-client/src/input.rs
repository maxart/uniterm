...
/// Remove terminal focus reports from keyboard input. Focus-in requests an
/// authoritative repaint; focus-out is consumed so neither report can leak
/// into a child application as typed escape bytes.
pub(crate) fn strip_focus_events(input: &[u8]) -> (bool, Vec<u8>) {
