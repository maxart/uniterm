...
CI runs the gates below on Linux and macOS for every pull request, plus the suite in release mode and the installer test; run them locally first. All must pass and clippy must be warning-free:
...
On macOS, run the suite with `TMPDIR=/tmp` so test socket paths stay under the Unix socket length limit; the default temp directory there is long enough to push them over it.
