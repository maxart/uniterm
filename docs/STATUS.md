...
|---|---|---|---|---|
| User-local installer and existing-install detection | Shipped | `install.sh` (`--system`, `UNITERM_INSTALL_DIR`) | `scripts/test-installer.sh` | Reuses `ut`/`uniterm` locations, defaults fresh installs to `~/.local/bin` or Termux's `$PREFIX/bin`, requires explicit sudo opt-in, and reports PATH conflicts |
...
| Inherited outer-terminal state recovery | Shipped | `uniterm_client::tty::TtyGuard::enable` / `Drop` | `attach_repairs_inherited_terminal_state_and_preserves_the_shell_screen` (CLI tests/terminal_startup.rs) | Attach selects ASCII and resets drawing modes before the first frame; detach preserves the shell screen and restores readable text |
| Agent-card context menu | Shipped | Right-click a docked Agents card; `Server::run_agent_context_action` | `agent_menu_focuses_copies_and_stops_only_the_selected_invocation` (tests/agent_context_menu.rs), `agent_menu_shares_visible_geometry_with_mouse_and_keyboard_actions` | Go to the exact Tab/Pane across Projects, copy the current working path, or stop one invocation through the shared agent-stop action |
...
| CLI output survives closed pipes | Shipped | `uniterm_cli::run`, `output::finish`, `cmd_agent` | `agent_explain_with_closed_stdout_returns_without_aborting`, `closed_stdout_and_stderr_do_not_panic`, `full_stdout_reports_an_error_without_panicking` (CLI tests/output_pipe.rs) | Closed output readers preserve command cleanup and status; other stdout failures report an error instead of panicking |
| CLI front door (`new-session`, `ls`, `attach`, `kill`) | Shipped | `uniterm_cli::cmd_new_session`, `cmd_ls`, `cmd_attach`, `cmd_kill` | `list_info_reflects_splits_and_windows_then_kill` (tests/m4_session.rs), `attach_aliases_share_target_defaults_and_validation` (CLI tests/attach_alias.rs) | PHASE1 M4. `attach`, `att`, and `a` share one handler |
...
| Chrome (status line, dividers) survives tested Pane operations | Shipped | `Server::draw_status`, `draw_dividers` | `status_line_survives_pane_ops` (tests/r2_chrome.rs) | R2. Reconstructs the client screen after attach, no-op, split, kill, zoom, Tab changes, resize, and each 2,000-line bottom-output burst; asserts exact status and divider positions |
...
| Width-aware damage renderer | Shipped | `Renderer::render`, `Renderer::render_row` | `damage_touching_a_wide_half_repaints_the_whole_glyph_span`, `full_and_incremental_output_match_an_independent_screen_oracle`, `blank_tail_erasure_stays_inside_the_pane_and_keeps_cursor_truthful`, `erasure_preserves_styled_blanks_and_resets_the_background` | DC6. Bounded ECH replaces default blank tails without erasing adjacent Panes; `render_spike` runs in CI |
| Chrome updates repaint in place, never full-frame, on agent activity | Shipped | `uniterm_server::server::chrome::Server::repaint_chrome_all` | `agent_binding_and_status_changes_never_clear_the_whole_screen` (tests/agent_status.rs) | R2/R3. Agent status, git stats, fleet membership, dev servers, and the file manager refresh the rails and status bar in place; only layout changes (splits, resize, zoom, overview) clear the screen. Fixes the sidebar flicker while an agent streams |
...
| Differential viewport comparison with tmux | Shipped | `Terminal::feed` compared to `tmux capture-pane` | `common_cli_stream_matches_tmux_viewport` (tests/vt_differential.rs) | DC8. Six fixtures at four chunk sizes, explicit local opt-in, required Linux/macOS CI execution; missing tmux fails the opted-in run. This is a bounded interoperability corpus, not complete VT conformance |
...
| Mouse wheel scrollback and alt-screen arrow emulation | Shipped | `Server::on_wheel` (server/mouse.rs), `Server::on_pty` foreground recovery (server/io.rs) | `wheel_scrolls_scrollback_and_returns_to_live`, `wheel_scrolls_history_emitted_by_an_inline_tui_region`, `wheel_reaches_direct_alt_screen_app_as_arrows`, `wheel_reaches_direct_alt_screen_app_as_application_arrows`, `wheel_reaches_direct_mouse_app_as_pane_relative_reports`, `wheel_returns_to_shell_history_after_a_stranded_foreground_app`, `claude_and_codex_alt_screen_wheels_and_typing_reach_the_app`, `claude_and_codex_inline_sessions_keep_scrollback_and_return_to_live` (tests/bf3_wheel.rs); opt-in `wheel_scrolls_inside_neovim` | BF3. First foreground observation preserves directly launched editors' alternate screen and mouse modes; recovery requires an observed foreground job returning to the pane child |
...
| Freeze on select: the screen holds still while text is selected | Shipped | `freeze-on-select` config key and Settings switch; `uniterm_server::copymode::CopyState::set_freeze_on_select`; `uniterm_core::grid::Grid::freeze_screen` | `freeze_on_select_holds_the_pane_while_output_scrolls`, `selection_without_freeze_on_select_shows_output_written_under_it` (tests/s1_selection.rs); `freeze_on_select_holds_the_screen_under_live_output`, `frozen_rows_keep_their_offset_when_history_is_evicted` (copymode.rs); `a_frozen_screen_survives_arena_compaction_and_later_output` (grid.rs) | Off by default. The screen rows are copied when a drag or `v` starts a selection and read from the copy until it ends; history above stays live and wheel-scrollable |
| Drag selection in apps that own the mouse (agent UIs, vim `mouse=a`) with a deferred plain click | Shipped | `freeze-on-select`; `Server::arm_selection`, `Server::finish_selection` (server/mouse.rs) | `freeze_on_select_takes_the_drag_from_a_mouse_mode_app_and_defers_clicks`, `a_mouse_mode_app_keeps_every_drag_without_freeze_on_select` (tests/s3_app_selection.rs) | With the switch on, a left-button drag is uniterm's frozen selection everywhere; the press is withheld until the release shows a plain click, which the app then receives whole. Off, the app owns every event as before |
| Copy on select (yank on release, or keep the selection on screen) | Shipped | `copy-on-select` config key and Settings switch; `CopyState::finish_mouse_selection` | `copy_on_select_off_keeps_the_selection_until_a_key_copies_or_a_click_dismisses_it` (tests/s3_app_selection.rs) | On by default. Off, a released selection stays highlighted in the frozen pane until `y` or Enter copies it, or Esc, `q`, or a plain click dismisses it |
...
| Capability | Status | Entry point | Evidence | Notes |
|---|---|---|---|---|
| Slow Workspace startup and restart | Shipped | `ut`, `uniterm`, `startup::wait_for_server`, `server::run_server_with_ready` | `startup_waits_past_the_old_deadline`, `startup_reports_failure_and_drains_more_than_a_pipe_of_diagnostics` (CLI startup.rs), `ordinary_start_restores_23_projects_and_44_tabs_and_restarts_after_clean_stop` (CLI tests/startup_recovery.rs) | One-shot readiness after recovery, no startup deadline or server kill; bounded diagnostics and a delayed progress notice; repair streams one record at a time |
...
| Grid and scrollback content persistence | Shipped | `Grid::capture_lines`, `runtime::persistence::Persistence::save_snapshot`, `persist::save_capture` | `compact_capture_matches_existing_bytes_after_source_mutation`, `compact_checkpoint_preserves_schema_and_all_event_cursors` (tests/checkpoint_capture.rs), `pty_output_advances_the_crash_snapshot_without_a_structural_change` | P2-2. Owned compact cells cross the seam; worker serialization preserves schema v12 without materializing per-cell Strings. Fleet allocation and latency budgets run in `checkpoint_spike` and CI |
...
| Corrupt-tail diagnosis and repair to the last consistent prefix | Shipped | `eventlog::repair` | `repair_keeps_the_prefix_before_a_partial_failed_append`, `repair_discards_a_gapped_suffix_but_never_a_future_schema`, `truncated_final_record_does_not_hide_the_last_complete_projection`, `repair_streams_across_small_buffers_and_propagates_read_errors` | Streaming validation keeps memory proportional to the largest record; healthy logs remain unchanged |
| Append-failure poisoning (no false checkpoint) | Shipped | `runtime::persistence::Persistence` | `failed_event_append_freezes_followups_and_snapshots` | |
| Acknowledged Workspace catalog saves and retry | Shipped | `Server::persist_workspace_definition`, `AgentToCore::WorkspaceCatalogSaved` | `failed_catalog_save_retries_the_same_definition_at_clean_stop`, `persistent_catalog_failure_retains_the_crash_checkpoint` (tests/catalog_retry.rs) | Failed writes do not enter the deduplication cache; clean stop retains its checkpoint if the catalog is unavailable |
| Bounded runtime transport with ordered events | Shipped | `runtime::outbox::Outbox`, `Server::runtime_read_ready` | `superseded_checkpoints_follow_every_ordered_event`, `full_transport_retains_a_bounded_ordered_prefix_without_blocking`, `shutdown_drains_a_full_reply_channel_and_flushes_ordered_work` | Coalesces queued observations and checkpoints, pauses readable sources under pressure, and drains bounded replies during shutdown |
| Event-driven closed-Pane reaping | Shipped | `Server::retire_pty`, `Server::flush_retiring_ptys` (server/reaper.rs) | `closing_a_pane_returns_before_grace_and_reaps_from_kernel_events` | TERM grace and KILL escalation never sleep on the live render loop; native exit watches disarm after teardown |
| Workspace advisory lock | Shipped | `WorkspaceLock::acquire` in `server/socket.rs` | `tests/workspace_lock.rs::workspace_lock_outlives_the_socket_path` (isolated from concurrent PTY forks) | |
...
| Agent discovery after same-PID launcher handoff | Shipped | `runtime::agent_main`, `CoreToAgent::PaneEvidence` | `detects_agent_after_same_pid_exec_without_idle_work` (tests/provider_exec.rs) | Process misses retry on changed screen/title evidence; no idle polling |
...
| Server-card context menu opens URLs, focuses source Panes, and stops listeners | Shipped | `server::dev_servers::Server::run_dev_server_action`, `dev_server::stop::stop_listener` | `server_menu_opens_focuses_and_kills_only_its_listener` (tests/server_context_menu.rs), `server_menu_keeps_all_three_actions_on_screen` | Shared card geometry and browser action; stable menu targets; runtime-side listener ownership checks and bounded TERM/KILL escalation |
...
| Background hierarchy and agent creation | Shipped | `ut project add`, `ut tab new`, `ut pane split`, `ut agent start --pane` with `--background`; targeted `pane close` and `tab close` | `tabs_can_be_created_and_named_by_hierarchy_position` (uniterm-cli/tests/tab_cli.rs) | Explicit background creation preserves selection; closing the selected resource chooses a replacement |
| Full-width shrinking Tabs | Shipped | `chrome::tab_bar_layout`, shared render and mouse slots | `tabs_fill_shrink_then_scroll_at_eight_cells`, `chrome_buttons_open_anchored_menus_and_plus_survives_tab_overflow` | Equal shares, eight-cell minimum, then horizontal scrolling |
| Portable prompt paste and submission | Shipped | `ut agent prompt`, `ut pane paste/prompt/submit`, `ut tab paste/prompt/submit`, `ut --skill`; control `pane_paste`, `tab_paste` | `tabs_can_be_created_and_named_by_hierarchy_position`, `enter_is_outside_multiline_paste_and_empty_submit_is_only_enter` | Bracketed paste when negotiated, Enter outside the paste; `--stdin` preserves multiline input |
...
| File opens avoid external-editor Tabs and slow Git discovery | Shipped | `Server::handle_file_action`, `runtime::launch_external_editor`, `GitWatchService`, `FileManager::click` | `first_click_opens_external_editor_without_a_tab_or_waiting_for_git`, `first_click_opens_unselected_rows_and_ignores_empty_space`, `projects_in_one_repository_share_one_watcher`, `directory_reads_do_not_trigger_refresh_but_file_changes_do` | Single-click open; native async child reaping; coalesced Git subscriptions and shared cached summaries; directory reads cannot refresh themselves |
| Reopening a file focuses its live terminal editor Pane | Shipped | `Server::on_agent_reply` (`EditorResolved`), `runtime::editor_reuses_tab` | `file_opens_reuse_live_editors_and_reopen_after_exit_or_close` (tests/file_editor.rs), `editor_tab_reuse_excludes_external_launchers` | Project-scoped canonical file identity; clears on editor completion or Pane close; external launchers run without a terminal Tab |
