...
The full data model for goals is in the maintainer notes (`09-uniterm-feature-port.md`, not published) (tier 1, item 4); here it is enough to know a goal is the thing an agent is working toward.
...
The full payload schema is frozen and published as a small versioned spec (see the maintainer notes (`12-open-questions.md`, not published) Q6), so third-party agents can target a stable contract; the `v` field carries the protocol version.
