...
Agents and Web servers default to the whole active Workspace and each has a Project/Workspace scope control; Workspace scope includes the owning Project on every row.
A saved scope selection takes precedence on restore; older catalogs without a selection use the Workspace default.
