...
- Reorder Tabs by dragging the Tab bar, using its menu or prefix `<` / `>`, or running focus-preserving background automation.
...
- A roomy top status and Tab bar that scrolls horizontally when needed, keeps its new-Tab button reachable, and adapts to live terminal resizes; Today hides Project tabs while its Workspace-level view is open.
- Workspace > Today shows a live visual day history of Projects, worktrees, agent invocations, and session IDs, with prior-day navigation, filters, notes, inspection, and validated resume.
- Optional authenticated encryption protects Workspace state with an external key; explicit per-submission prompt retention requires protected storage.
...
- A docked Observatory keeps agents, Project file access, and detected web servers beside the terminal, with direct Pane focus and workspace-safe actions; Agents and Servers default to the current Workspace, with saved scope selections preserved.
- A Today manager can read activity metadata and monitor the Workspace through a read-only endpoint by default; management access is explicit.
